...
\begin{equation}
    ...
    \int f(\nu) d\nu = 1.
\end{equation}
Several fits in the literature obey this relation \citep[eg.][]{Sheth2001,Peacock2007,Tinker2010}, \bd{Tinker+08 rather than 10? They have a whole appendix on satisfying the integral constraint.} \sgm{I don't think I've implemented the normalisable form of Tinker08. Tinker10 also has a normalisable form.} which ensures that all mass is bound in halos. \bd{I think for the purposes of the HM it IS a requirement, no?} \sgm{I dunno. Technically, yes, but practically?}

Figure \ref{fig:mfs} shows the HMF for four of the fits available in \textsc{halomod} (via \textsc{hmf}). \bd{The HaloMod term has barely been defined?} \sgm{Not sure what to do about that yet} According to these fitting functions, the HMF behaves as a power-law at low masses, with an exponential cut-off above a typical mass $m_* \approx 5\times10^{14}$ (this mass changes with redshift). There is clearly significant variations between the fits, especially at high masses, for a range of reasons, including halo finder differences, alternate mass definitions, and simulation resolution \citep[cf.][]{Murray2013a,Knebe2011}. 

\begin{figure}
  \centering
  \includegraphics[width=\linewidth]{figures/dndm_mffits.pdf}
  \caption[Several HMF fits]{Four HMFs from the literature, including the `original' form of \protect\citet[]['PS']{Press1974}, motivated by spherical collapse, \textsc{SMT} motivated by ellipsoidal collapse from \protect\citet{Sheth2001}) as well as fits from \protect\citep{Jenkins2001,Tinker2008}}.}
  \label{fig:mfs}
\end{figure}
...
%Efficiency, while important, is not the primary aim of \verb|halomod|. As a broad approach, we have aimed for flexibility and ease-of-use, along with a robust design, over efficiency. This is made clear from the outset by the choice of programming language, as Python is known for being relatively slow in several regimes. We believe that flexibility and robust design will be of much greater benefit for the community and the future of modelling with the HM than raw efficiency. Despite this, with the era of `big data' upon us it is important to optimize wherever possible. We have done this in two ways: at a functional level, and an algorithmic level. \bd{I'd more or less cut the beginning of the paragraph, it sounds defensive. We could just say ``.''}
...
%Summarily, the \verb|Component| architecture of \textsc{halomod} is geared to provide flexibility and extendibility for crucial developments in the field in the years to come.
...
{\color{red} We refer to table 3 in Chapter 2} for a table of fitting functions, noting that four more fits have been added to the collection, specifically those of \citet{Tinker2010,Manera2010,Pillepich2010} and \citet{Ishiyama2015}. 
...
We summarise the API of the \verb|Bias| component in table \ref{tab:component_bias}, and compile the range of included models in table \ref{tab:models_bias}. Figure \ref{fig:bias_functions} displays a selection of the available models.
...
\citet{Sheth2001a} & $\displaystyle   p(\kappa) = \frac{1}{4} \left[-|\kappa| (2 \text{Ci}|\kappa| \sin|\kappa|+\pi\cos(\kappa)) \right.
 ...
 % & $\displaystyle  l(x,c) = Sheth $\\
 %%% CAN GET HERNQUIST SELF-CONVOLUTION FROM SHETH ET AL. 2001a
 \midrule
...
%\texttt{Uniform} & $ \displaystyle f(x) = 1 $\\
% & $ \displaystyle   h(c) = c^3/3 $\\
% & $ \displaystyle   p(\kappa,c) = \frac{\sin(c\kappa) - c\kappa \cos(c\kappa)}{\kappa^3} $ \\
%    
\bottomrule[0.05cm]
\end{tabu}
...
% Again exploiting the homogeneity of the core \verb|Framework| structure, we are able to present a unified interface to any \verb|Framework| and an arbitrary selection of its properties. The features of the fitting capabilities include:
% \begin{itemize}
%     \item CLI or functional interface
%     \item Fit to any quantity from any any \verb|Framework|
%     \item Completely specify the base model parameters (with defaults given by normal usage).
%     \item Arbitrary specification of variable parameters to be fit (including elements of any of the \verb|Component| parameter dictionaries). 
...
%\section{TheHaloMod: A new web-interface}
%\label{sec:thehalomod}
%The Halo Model has become an indispensable tool for cosmological spatial statistics analysis in the past decade. Consequently, its use is becoming more widespread, with interest from those who are not necessarily core to its development. Such a context is an excellent motivation for creating a web-based implementation which is readily available with no installation requirements, and an intuitive interface.
%
