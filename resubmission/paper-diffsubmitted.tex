...
%DIF DEL paper-oldtmp-267207.tex   Thu Jul  8 13:37:45 2021
%DIF ADD paper.tex                 Thu Jul  8 13:28:53 2021
...
%DIF 26a29-31
 %DIF > 
\creflabelformat{equation}{#2\textup{#1}#3} %DIF > 
 %DIF > 
%DIF -------
\makeatletter
...
%DIF -------
\newcommand{\Nc}{\langle N_{\rm c} \rangle} %DIF > 
\newcommand{\Ns}{\langle N_{\rm s} \rangle} %DIF > 
\newcommand{\Nt}{\langle N_{\rm t} \rangle} %DIF > 
\newcommand{\halomod}{\textsc{halomod}\xspace} %DIF > 
\newcommand{\python}{\textsc{python}\xspace} %DIF > 
\newcommand{\thm}{\textsc{TheHaloMod}\xspace} %DIF > 
\newcommand{\hmf}{\textsc{hmf}\xspace} %DIF > 
\newcommand{\framework}{\texttt{Framework}\xspace} %DIF > 
\newcommand{\component}{\texttt{Component}\xspace} %DIF > 
\newcommand{\parameter}{\texttt{parameter}\xspace} %DIF > 
\newcommand{\cached}{\texttt{cached\_quantity}\xspace} %DIF > 
%DIF -------
...
%DIF 78a85-86
\newcommand{\redbf}[1]{\textcolor{red}{\textbf{#1}}} %DIF > 
...
\author{Steven~G.~Murray$^a$, Benedikt Diemer$^b$, Zhaoting Chen$^c$\DIFaddbegin \DIFadd{, Anton Glenn Neuhold Jr.$^d$, Alexander Schnapp$^d$, Tia Peruzzi$^d$, Daniel Blevins$^d$, Trent Engelman$^d$}\DIFaddend }
\DIFdelbegin %DIFDELCMD < \address{$^a$ Arizona State University, Tempe, AZ, USA \\
%DIFDELCMD < $^b$ Department of Astronomy, University of Maryland, College Park, MD 20742, USA\\
%DIFDELCMD < $^c$ Jodrell Bank Centre for Astrophysics, School of Physics and Astronomy, The University of Manchester, Manchester M13 9PL, UK}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \address{$^a$ School of Earth and Space Exploration, Arizona State University, Tempe, AZ, USA \\
...
$^d$ ASU School of Computing, Informatics, and Decision Systems Engineering, Arizona State University, Tempe, AZ, USA \\
}
\DIFaddend 
...
\begin{abstract}
The halo model is a successful framework for describing the distribution of matter in the Universe -- from weak lensing observables to galaxy \DIFdelbegin \DIFdel{2-point }\DIFdelend \DIFaddbegin \DIFadd{n-point }\DIFaddend correlation functions. We review the basic formulation of the halo model and several of its components in the context of galaxy \DIFdelbegin \DIFdel{two-point }\DIFdelend \DIFaddbegin \DIFadd{2-point }\DIFaddend statistics, developing a coherent framework for its application.

We use this framework to motivate the presentation of a new \DIFdelbegin \DIFdel{Python }\DIFdelend \DIFaddbegin \python \DIFaddend tool for simple and efficient calculation of halo model quantities, and their extension to galaxy statistics via a \textit{halo occupation distribution}, called \halomod. This tool is efficient, simple to use, comprehensive and importantly provides a great deal of flexibility in terms of custom extensions. 
This \DIFdelbegin \DIFdel{Python }\DIFdelend \DIFaddbegin \python \DIFaddend tool is complemented by a new web-application at \url{https://thehalomod.app} that supports the generation of many halo model quantities directly from the browser -- useful for educators, students, theorists and observers.
...
The halo model \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{Neyman1953,Peacock2000,Seljak2000,Ma2000,Cooray2002} }\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{Neyman1953,Scherrer1991,Peacock2000,Seljak2000,Ma2000,Cooray2002} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend is an enormously successful analytical description of the large-scale distribution of matter in our Universe.
...
\citep{Zehavi2011}, from real-space or projected \DIFdelbegin \DIFdel{two-point }\DIFdelend \DIFaddbegin \DIFadd{2-point }\DIFaddend correlation functions (2PCFs), to galaxy-galaxy lensing, to higher-order correlations.  

In practice, the HOD formalism has been widely used in the interpretation of galaxy populations in the past decade. Most of these studies have focused on determining the parameters of the HOD (i.e. the galaxy-halo connection) from the 2PCF of galaxies \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep[eg.][]{Moustakas2002,Bullock2002,Zheng2004,Zehavi2005,Blake2008,Zehavi2011,Beutler2013,Skibba2015} }\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep[eg.][]{Moustakas2002,Bullock2002,Zheng2004,Zehavi2005,Blake2008,Zehavi2011,Beutler2013,Skibba2015,Rodriguez-Torres2015,Nicola2020,Zhou2021, Ishikawa2021} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend using the analytical framework that we present in this paper. However, other observables such as galaxy-galaxy lensing \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{Mandelbaum2006}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{Mandelbaum2006,Cacciato2012,Clampitt2017,Dvornik2018}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend , the 2PCF of radio galaxies \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{Wake2008a,Kim2011}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{Wake2008a,Kim2011,Nusser2015}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend , galaxy-quasar cross-correlations \citep{Shen2013}, near-UV cross-correlations \citep{Krause2012}\DIFaddbegin \DIFadd{, Fast Radio Bursts \mbox{%DIFAUXCMD
\citep[FRBs;][]{Rafiei-Ravandi2020} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend and \textsc{Hi} intensity map cross-correlations \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{Padmanabhan2016b,Wolz2019} }\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{Padmanabhan2016b,Wolz2019,Chen2021} }\hspace{0pt}%DIFAUXCMD
and auto-correlations \mbox{%DIFAUXCMD
\citep{Schneider2020,Umeh2021} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend have also received application through the same framework. Furthermore, several studies have successfully combined observables \DIFaddbegin \DIFadd{(either in cross-correlation or via joint likelihoods) }\DIFaddend to break degeneracies \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{Leauthaud2011,Leauthaud2012,More2013}}\hspace{0pt}%DIFAUXCMD
, and }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep[eg.][]{Leauthaud2011,Leauthaud2012,More2013,Schaan2021}}\hspace{0pt}%DIFAUXCMD
. 
Finally, }\DIFaddend the combination of the galaxy 2PCF with \DIFaddbegin \DIFadd{other statistics, eg. }\DIFaddend weak-lensing \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{More2015}}\hspace{0pt}%DIFAUXCMD
, }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{More2015,Miyatake2020}}\hspace{0pt}%DIFAUXCMD
, cluster counts and auto-correlations \mbox{%DIFAUXCMD
\citep{To2021}}\hspace{0pt}%DIFAUXCMD
, or both \mbox{%DIFAUXCMD
\citep{To2020}}\hspace{0pt}%DIFAUXCMD
, }\DIFaddend stellar mass functions \citep{Coupon2015}
 and group mass-to-number ratios \citep{Reddick2014} has resulted in the ability to simultaneously constrain cosmological parameters along with those of the HOD.
...
There are a few publicly available implementations of the \DIFaddbegin \DIFadd{analytic }\DIFaddend HM, many of which were used in the testing of our code \DIFdelbegin \DIFdel{although the development of most of them has been discontinued }\DIFdelend (e.g., \textsc{chomp}\footnote{\DIFdelbegin %DIFDELCMD < \url{https://code.google.com/p/chomp/}  %%%
\DIFdelend \DIFaddbegin \url{https://github.com/morriscb/chomp}  \DIFaddend -- discontinued.}, \textsc{HMcode}\footnote{\url{https://github.com/alexander-mead/HMcode/}}\DIFaddbegin \DIFadd{, }\textsc{\DIFadd{CCL}}\footnote{\url{https://github.com/LSSTDESC/CCL}} \DIFaddend and \textsc{AUM}\footnote{\url{https://github.com/surhudm/aum}\DIFdelbegin \DIFdel{-- discontinued.}\DIFdelend }).
\DIFaddbegin \DIFadd{In addition, methods of computing HM quantities based on emulation from simulations are becoming available (eg. }\textsc{\DIFadd{DarkQuest}}\footnote{\url{https://github.com/DarkQuestCosmology/dark_emulator_public}} \DIFadd{and }\textsc{\DIFadd{CosmicEmu}}\footnote{\url{https://github.com/lanl/CosmicEmu}} \DIFadd{-- though the latter only supports matter correlations).
}\DIFaddend %, nor that they are poor implementations in general (indeed, all of these were useful in testing our own code). 
...
\DIFaddbegin \DIFadd{Additionally, existing codes -- where still in active development -- do not tend to offer a great deal of flexibility in the composition and extension of 
the various components and models involved in the halo model, rather focusing on a limited set of well-honed models and tuned for maximum performance.
}\DIFaddend 
...
Both philosophically and technically, \halomod\ inherits from the \hmf\ halo mass function package\footnote{\DIFdelbegin %DIFDELCMD < \url{https://github.com/steven-murray/hmf}%%%
...
\begin{figure*}
    \centering
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\linewidth]{thehalomod_app_schematic.pdf} 
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\linewidth,clip=true,trim=0 2cm 0 1cm]{screenshot_thm.pdf} 
    \DIFaddendFL \caption{\textsc{TheHaloMod} in action. The main page (right panel) shows the currently computed models, with a selection of quantities to plot, and a number of actions available for each model (edit/clone/report bug/delete). The \DIFaddbeginFL \DIFaddFL{plot itself is interactive. The }\DIFaddendFL input form (\DIFdelbeginFL \DIFdelFL{two insets on the }\DIFdelendFL left) provides a wide range of options, including all of the components presented throughout this paper. Each component \DIFdelbeginFL \DIFdelFL{is defined in }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{has }\DIFaddendFL its own \DIFdelbeginFL \DIFdelFL{tab}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{section}\DIFaddendFL . \DIFdelbeginFL \DIFdelFL{Those shown here are cosmology }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{The interface features responsive design }\DIFaddendFL and \DIFdelbeginFL \DIFdelFL{the HOD model}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{a dark mode}\DIFaddendFL .} \label{fig:webpage-image}
\end{figure*}

This paper is structured as follows: \S\ref{sec:DMHaloModel} details the theory of the HM particularly in the context of dark matter \DIFdelbegin \DIFdel{two-point }\DIFdelend \DIFaddbegin \DIFadd{2-point }\DIFaddend statistics, collating the various components involved in a manner consistent with our implementation. Following this, we describe how to extend the halo model to tracer populations in \S\ref{sec:TracerHaloModel}. Then, we describe our code and its usage in  \S\ref{sec:halomod}, and in \S\ref{sec:applications} we present an illustrative example. In \S\ref{sec:future} we define a prospectus for the future, before summarising and concluding in \S\ref{sec:summary}.

Note that the code to produce all figures of different component models in this paper, as well as the example application, are available publicly as examples in \halomod's documentation\footnote{At \url{https://halomod.readthedocs.io/en/latest/examples/component-showcase.html} and \url{https://halomod.readthedocs.io/en/latest/examples/fitting.html} respectively.}. This paper refers to \textsc{hmf} version \DIFdelbegin \DIFdel{3.2}\DIFdelend \DIFaddbegin \DIFadd{3.4}\DIFaddend .1 and \textsc{halomod} version \DIFdelbegin \DIFdel{2.0}\DIFdelend \DIFaddbegin \DIFadd{2.1}\DIFaddend .0.


\section{The Dark Matter Halo Model}
\label{sec:DMHaloModel}
The broad assumption underpinning the HM is that in hierarchical structure formation scenarios, all mass is expected to be bound to halos at some scale\footnote{This assumption is clearly an approximation, since even cold dark matter (CDM) has a free-streaming scale in the early Universe below which we expect non-virialized mass \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{Frenk2012,Schneider2014a}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{Frenk2012,Schneider2015}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend }. If this is the case, then the entire nonlinear density field may be reconstructed by summing contributions from the individual halos. If, in addition, we may describe the average radial density profile of halos as spherically-symmetric, with a shape that depends solely on the mass of the halo itself, we can write:
\begin{equation}
    \label{eq:hm_sum}
...
The application of the HM rests in converting the prem\-ise of \DIFdelbegin \DIFdel{Eq. \ref{eq:hm_sum} }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cref{eq:hm_sum} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend into a semi-analytic integration. This requires knowledge of four key components:
\begin{enumerate}
    \item The \DIFdelbegin \DIFdel{average radial density profile }\DIFdelend \DIFaddbegin \DIFadd{moments of the density profiles }\DIFaddend of halos, \DIFdelbegin \DIFdel{$\rho(r|m)$
    }\DIFdelend \DIFaddbegin \DIFadd{$\langle \rho(r|m) \rangle$, $\langle \rho^2_{h, i}(r|m) \rangle$, etc.
    }\DIFaddend \item The abundance of halos of a given mass (termed the halo mass function, or HMF), $n(m)$
    \item The expected overdensity of halos of mass $m$, \DIFdelbegin \DIFdel{$\delta_h(\mathbf{x},m)$ }\DIFdelend \DIFaddbegin \DIFadd{$\Delta_{\rm h}(\mathbf{x},m)$ }\DIFaddend in a region with a given overdensity of matter $\delta(\mathbf{x})$, called the \textit{halo bias}.
    \item A model for the \DIFdelbegin \DIFdel{linear }\DIFdelend spatial distribution of matter, $\delta(\mathbf{x})$.
\end{enumerate}
Furthermore, detailed calculations require the following extra components:
\begin{enumerate}
    \DIFdelbegin %DIFDELCMD < \setcounter{enumi}{3}
%DIFDELCMD <     %%%
\DIFdelend \DIFaddbegin \setcounter{enumi}{4}
    \DIFaddend \item Additional parameters that describe the halo profile, most
...
Although the halo model can be used to describe the density field at any level of the $n$-point hierarchy, its most common application to date has been to compute \DIFdelbegin \DIFdel{two-point }\DIFdelend \DIFaddbegin \DIFadd{2-point }\DIFaddend statistics. Thus, in this outline, we shall focus on the framework as it pertains to the 2PCF.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{In particular, the 2PCF only requires the first two moments of the density profile, and we will furthermore assume that the second moment is just the square of the average density profile (i.e. we only require the first moment, cf. \mbox{%DIFAUXCMD
\citealt{Giocoli2010}}\hspace{0pt}%DIFAUXCMD
).
}\DIFaddend Our aim in this section is to introduce the theory in a manner conducive to our implementation, so we shall cover each of the six components in turn following the presentation of the core framework.


Throughout the section, we discuss isotropic \DIFdelbegin \DIFdel{two-point }\DIFdelend \DIFaddbegin \DIFadd{2-point }\DIFaddend statistics, which measure the over-density of pairs of points at a certain scalar separation. This can be formulated either in Euclidean space, as the correlation function $\xi(r)$, or in Fourier-space, as the power spectrum $P(k)$. 
...
\begin{equation}
    \xi(\mathbf{r}) = \frac{1}{V}\int_V \DIFaddbegin {\rm \DIFaddend d\DIFaddbegin }\DIFaddend ^3\mathbf{x}\ \delta(\mathbf{x}) \delta(\mathbf{x} - \mathbf{r}),
\end{equation}
...
\begin{equation}
    \xi(\mathbf{r}) = \int \DIFdelbegin \DIFdel{\frac{d^3 \mathbf{k}}{(2\pi)^3} }\DIFdelend \DIFaddbegin \DIFadd{\frac{{\rm d}^3 \mathbf{k}}{(2\pi)^3} }\DIFaddend P(\mathbf{k}) e^{i \mathbf{k}\cdot \mathbf{r}}.
\end{equation}
...
\begin{equation}
\label{eq:hankel}
\xi(r) = \frac{1}{2\pi^2}\int_0^\infty P(k)k^2 j_0(kr)\DIFdelbegin \DIFdel{dk}\DIFdelend \DIFaddbegin {\rm \DIFadd{d}}\DIFadd{k}\DIFaddend ,
\end{equation}
where $j_0$ is the zeroth-order \textit{spherical} Bessel function,
\begin{equation}
    j_0(x) = \frac{\sin x}{x}.
\end{equation}
We note that the halo model is \textit{not limited} to such \DIFdelbegin \DIFdel{two-point }\DIFdelend \DIFaddbegin \DIFadd{2-point }\DIFaddend statistics, but our implementation currently is.

\subsection{Clustering Framework}
\label{sec:haloclustering}
It is convenient to formulate the framework of clustering in two regimes \citep{Seljak2000}, intra- and inter-halo pairs, called the 1-halo and 2-halo terms respectively. These approximately correspond to small- and large-scale structure (i.e. the contribution of each in the opposite regime is negligible), where `small' is sub-megaparsec, and `large' is \DIFdelbegin \DIFdel{$>\sim5h^{-1}$}\DIFdelend \DIFaddbegin \DIFadd{$\gtrsim5h^{-1}$}\DIFaddend Mpc. 
...
\begin{equation}
    P(k) = P\DIFdelbegin \DIFdel{^{1h}}\DIFdelend \DIFaddbegin \DIFadd{^{\rm 1h}}\DIFaddend (k) + P\DIFdelbegin \DIFdel{^{2h}}\DIFdelend \DIFaddbegin \DIFadd{^{\rm 2h}}\DIFaddend (k),
\end{equation}
and
\begin{equation}
    \xi(r) = [1 + \xi\DIFdelbegin \DIFdel{^{1h}}\DIFdelend \DIFaddbegin \DIFadd{^{\rm 1h}}\DIFaddend (r)] + \xi\DIFdelbegin \DIFdel{^{2h}}\DIFdelend \DIFaddbegin \DIFadd{^{\rm 2h}}\DIFaddend (r).
\end{equation}
...
\begin{equation}
\label{eq:dmcorr1}
1 + \xi\DIFdelbegin \DIFdel{_{DM}^{1h}}\DIFdelend \DIFaddbegin \DIFadd{_{\rm DM}^{\rm 1h}}\DIFaddend (r) = \DIFaddbegin \DIFadd{\frac{1}{\bar{\rho}^2}}\DIFaddend \int n(m) \DIFdelbegin %DIFDELCMD < \left(%%%
\DIFdel{\frac{m}{\bar{\rho}}}%DIFDELCMD < \right)%%%
\DIFdel{^2 }\DIFdelend \lambda(r|m) \DIFdelbegin \DIFdel{dm}\DIFdelend \DIFaddbegin {\rm \DIFadd{d}}\DIFadd{m}\DIFaddend ,
\end{equation}
where $\lambda$ is the \DIFdelbegin \DIFdel{mass-normalised }\DIFdelend self-convolution \DIFaddbegin \DIFadd{of the halo profile }\DIFaddend (i.e. the 3D integral of the profile multiplied by itself after a shift of length $r$, \DIFaddbegin \DIFadd{which has units of $M^2/V$, }\DIFaddend cf. \S\ref{sec:profilestheory}), and $n(m)$ is the halo mass function (HMF).

It is common to express this in Fourier-space, since the self-convolution becomes a simple multiplication,
\begin{equation}
    \label{eq:dmpower1}
    P\DIFdelbegin \DIFdel{_{DM}^{1h}}\DIFdelend \DIFaddbegin \DIFadd{_{\rm DM}^{\rm 1h}}\DIFaddend (k) = \int n(m) \left(\frac{m}{\bar{\rho}}\right)^2 |u(k|m)|^2 \DIFdelbegin \DIFdel{dm}\DIFdelend \DIFaddbegin {\rm \DIFadd{d}}\DIFadd{m}\DIFaddend ,
\end{equation} 
where $u(k|m)$ is the \DIFdelbegin \DIFdel{normalised }\DIFdelend \DIFaddbegin \DIFadd{mass-normalised }\DIFaddend Fourier transform of the halo profile \DIFaddbegin \DIFadd{(which is unitless and approaches unity as $k\rightarrow0$)}\DIFaddend . This form has the distinct advantage that higher-order correlations merely involve an increase in the exponent of $u$, rather than an intractable multi-dimensional convolution. Conversely, for numerical purposes, if an analytic formulation of the self-convolution exists and the intended quantity is the correlation function, then it is more efficient to use \DIFdelbegin \DIFdel{Eq. \ref{eq:dmcorr1} }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cref{eq:dmcorr1} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend directly. 

\DIFaddbegin \DIFadd{Note that the behaviour as $k\rightarrow0$ is to tend to a constant:
}\begin{equation}
    \DIFadd{P_{\rm DM}^{\rm 1h}(k\rightarrow0) = \frac{1}{\bar{\rho}^2} \int n(m) m^2\ {\rm d}m.
}\end{equation}
\DIFadd{This ``shot-noise-like'' term \mbox{%DIFAUXCMD
\citep{Cooray2002,Ginzburg2017} }\hspace{0pt}%DIFAUXCMD
arises due to exclusion features of the halo model, and is spurious. 
At the largest scales, it comes to dominate the 2-halo term.
While true shot-noise is inversely proportional to the }\textit{\DIFadd{number density}} \DIFadd{of the field (cf. \S\ref{sec:theory-gal:framework}), in this case we are considering the dark matter field to be essentially continuous (i.e. the mass of each DM `particle' approaches zero, and its number density approaches infinity), and it therefore should have no shot-noise. 
We thus offer an option }\texttt{\DIFadd{force\_1halo\_turnover}} \DIFadd{in }\textsc{\DIFadd{halomod}} \DIFadd{that forcibly removes this large-scale structure from the 1-halo term. 
}


\DIFaddend \subsubsection{2-halo term}
\label{sec:theory:2halo}
The two-halo term is most elegantly expressed in Fourier-space, 
\begin{align}
    P_{\rm DM}\DIFdelbegin \DIFdel{^{2h}}\DIFdelend \DIFaddbegin \DIFadd{^{\rm 2h}}\DIFaddend (k,r) =  \int \int & \DIFdelbegin \DIFdel{dm}\DIFdelend \DIFaddbegin {\rm \DIFadd{d}}\DIFadd{m}\DIFaddend _1 \DIFdelbegin \DIFdel{dm}\DIFdelend \DIFaddbegin {\rm \DIFadd{d}}\DIFadd{m}\DIFaddend _2\  I_{\rm DM}(k,m_1) I_{\rm DM}(k,m_2) \nonumber \\ 
    &\times P\DIFdelbegin \DIFdel{_{hh}}\DIFdelend \DIFaddbegin \DIFadd{_{\rm hh}}\DIFaddend (k,r|m_1,m_2),
\end{align}
where
\begin{equation}
    I_{\rm DM}(k,m) = n(m) u(k|m) \frac{m}{\bar{\rho}}.
\end{equation}
\DIFdelbegin \DIFdel{Here, $P_{hh}(k,r|m_1,m_2)$ }\DIFdelend \DIFaddbegin \DIFadd{This equation can be confusing at first glance because it seems to be a function of both $k$ and $r$ -- which are themselves not independent variables. We shall resolve this confusion presently.
$P_{\rm hh}(k,r|m_1,m_2)$ }\DIFaddend is the power spectrum of halo centres for halos of mass $m_1$ and $m_2$. This is in general a complicated function of mass and scale, but the most common approach is to approximate it by a first-order linear bias of the matter power,
\begin{equation}
    \label{eq:halo_centre_power}
    P\DIFdelbegin \DIFdel{_{hh}}\DIFdelend \DIFaddbegin \DIFadd{_{\rm hh}}\DIFaddend (k,r|m_1,m_2) \approx b(m_1,r)b(m_2,r)P\DIFdelbegin \DIFdel{_m}\DIFdelend \DIFaddbegin \DIFadd{_{\rm m}}\DIFaddend (k),
\end{equation}
where $b(m,r)$ is the first-order bias at $m$ (cf. \S\ref{sec:biastheory}), possibly with a scale-dependent correction, and \DIFdelbegin \DIFdel{$P_m(k)$ }\DIFdelend \DIFaddbegin \DIFadd{$P_{\rm m}(k)$ }\DIFaddend is the matter power spectrum (cf. \S\ref{sec:powerspec})\footnote{Note that \DIFdelbegin \DIFdel{thoughout}\DIFdelend \DIFaddbegin \DIFadd{throughout}\DIFaddend , we use a subscript $m$ to denote either the linear matter power or the nonlinear matter power derived using analytic corrections, as opposed to the full halo model.}. \DIFdelbegin \DIFdel{Typically, }\DIFdelend \DIFaddbegin \DIFadd{Depending on the application, it is not uncommon \mbox{%DIFAUXCMD
\citep[eg.][]{Cooray2002, Smith2011} }\hspace{0pt}%DIFAUXCMD
to adopt for $P_{\rm m}$ }\DIFaddend empirical nonlinear corrections on the linear matter power spectrum from \textsc{halofit} \citep{Smith2003}\DIFdelbegin \DIFdel{are used for this quantity.
}\DIFdelend \DIFaddbegin \footnote{\DIFadd{This is slightly circular, as the }\textsc{\DIFadd{halofit}} \DIFadd{model itself is inspired by the halo model (and is tuned to emulate the clustering of dark matter within and between halos).}}\DIFadd{. 
The motivation for using the nonlinear matter power here is to obtain better agreement with simulations at quasi-linear scales, where the power is suppressed by a number of effects, including mutual exclusion of individual halos and the breakdown of linear theory (understandably) at the precise scales at which halos are forming \mbox{%DIFAUXCMD
\citep{Mead2021}}\hspace{0pt}%DIFAUXCMD
.
Unfortunately, these somewhat circular corrections are not self-consistent -- adding contributions from the 1-halo term into the power for the 2-halo term.
Thus corrections made in the scale-dependent bias (which fundamentally should account for the halo exclusion, cf. \ref{sec:theory:exclusion}) are then forced to do two jobs: to model the true effect of halo exclusion, and to undo the fudged nonlinear corrections.
In principle, it is better to keep these effects separate, and deal self-consistently with the latter when modelling $P_{\rm m}$.
Thus, studies of pure matter correlations most often adopt either the linear matter power spectrum, or corrections from perturbation theory or similar \mbox{%DIFAUXCMD
\citep[eg.][]{Seljak2015,Mead2021}}\hspace{0pt}%DIFAUXCMD
.
These extensions partially correct the quasi-linear regime in a self-consistent way. 
Still other studies use emulated halo-halo power spectra directly, and thus remove the need to model the one-to-two-halo transition entirely \mbox{%DIFAUXCMD
\citep[eg.][]{Nishimichi2021}}\hspace{0pt}%DIFAUXCMD
.
}\DIFaddend We leave its definition free in our implementation \DIFdelbegin \footnote{\DIFdel{This is slightly circular, as the }\textsc{\DIFdel{halofit}} %DIFAUXCMD
\DIFdel{model itself uses the halo model. However, this is a common practice in halo model calculations, eg. \mbox{%DIFAUXCMD
\citep{Cooray2002, Smith2011}}\hspace{0pt}%DIFAUXCMD
}}%DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{(with a default of `linear'), and caution the user to pay close attention to its value for any particular application}\DIFaddend .

The general form for the 2-halo term then becomes
\begin{align}
\label{eq:dmpower2a}
    P_{\rm DM}\DIFdelbegin \DIFdel{^{2h}}\DIFdelend \DIFaddbegin \DIFadd{^{\rm 2h}}\DIFaddend (k,r) =  \int\int & I_{\rm DM}(k,m_1) I_{\rm DM}(k,m_2) \nonumber \\
    &\times b(m_1,r)b(m_2,r)P\DIFdelbegin \DIFdel{_{m}}\DIFdelend \DIFaddbegin \DIFadd{_{\rm m}}\DIFaddend (k) \DIFdelbegin \DIFdel{dm}\DIFdelend \DIFaddbegin {\rm \DIFadd{d}}\DIFadd{m}\DIFaddend _2 \DIFdelbegin \DIFdel{dm}\DIFdelend \DIFaddbegin {\rm \DIFadd{d}}\DIFadd{m}\DIFaddend _1
\end{align}
...
\begin{equation}
    \label{eq:dmpower2}
    P\DIFdelbegin \DIFdel{_{DM}^{2h}}\DIFdelend \DIFaddbegin \DIFadd{_{\rm DM}^{\rm 2h}}\DIFaddend (k,r) = P\DIFdelbegin \DIFdel{_m}\DIFdelend \DIFaddbegin \DIFadd{_{\rm m}}\DIFaddend (k) \left[ \int  I_\text{DM}(k,m) b(m,r) \DIFdelbegin \DIFdel{dm }\DIFdelend \DIFaddbegin {\rm \DIFadd{d}}\DIFadd{m }\DIFaddend \right]^2.
\end{equation}
\DIFaddbegin \DIFadd{The right-most factor is often called the ``effective bias'' of matter, and under the halo model ansatz must by definition approach unity as $k\rightarrow0$ (since matter is not biased against itself at large scales). In practice, several effects can artificially inhibit this behaviour, and we provide mechanisms for dealing with these in }\textsc{\DIFadd{halomod}} \DIFadd{(cf. \S\ref{sec:halomod:frameworks:base}).
}\DIFaddend 
...
\DIFdelbegin \DIFdel{Figure \ref{fig:power} }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cref{fig:power} }\hspace{0pt}%DIFAUXCMD
...
In fact, all curves (except that of \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citet{Bond1984}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citealt{Bond1984}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend ) are within $\sim 10\%$ over the plotted range.

\begin{figure}
  \centering
  \includegraphics[width=\linewidth]{transfer_models.pdf}
  \caption[Comparison of transfer function models]{A selection of transfer function models, shown as linear power spectra, including analytic approximations \citep{Bond1984,Bardeen1986,Eisenstein1998} and Boltzmann solutions. Details for each model (referenced to the alias given in the legend) can be found in Table \ref{tab:models_transfer}. The general shape is reproduced in each case, with details of baryon acoustic oscillations differing. In the bottom panel, we show the relative fraction compared to the CAMB transfer function, which renders all models without baryons as having oscillations on the acoustic scales. \DIFaddbeginFL \DIFaddFL{Note that the models are normalized to the same value of $\sigma_8$ (though some models, like BBKS, appear to be low across most of the range, their brief excursion above CAMB in the centre is highly weighted).}\DIFaddendFL }
  \label{fig:power}
\end{figure}

The calculation of the halo-centre power spectrum is commonly approximated by \DIFdelbegin \DIFdel{Eq. \ref{eq:halo_centre_power}}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cref{eq:halo_centre_power}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend , i.e.
\begin{equation*}
    P\DIFdelbegin \DIFdel{_{hh}}\DIFdelend \DIFaddbegin \DIFadd{_{\rm hh}}\DIFaddend (k,r|m_1,m_2) \approx b(m_1,r)b(m_2,r)P\DIFdelbegin \DIFdel{_m}\DIFdelend \DIFaddbegin \DIFadd{_{\rm m}}\DIFaddend (k),
\end{equation*}
 in which an estimate of the matter power spectrum is linearly biased. \DIFdelbegin \DIFdel{Typically, a }\textit{\DIFdel{nonlinear}} %DIFAUXCMD
\DIFdel{estimate of the matter power is used, for which the corrections of \mbox{%DIFAUXCMD
\citet[][hereafter \textsc{halofit}]{Smith2003} }\hspace{0pt}%DIFAUXCMD
are employed}\DIFdelend \DIFaddbegin \DIFadd{As previously discussed, it is not uncommon to use the non-linear power spectrum from }\textsc{\DIFadd{halofit}} \DIFadd{to model $P_m$}\DIFaddend . We do not present the details here, except to mention that the modifications are applied \DIFaddbegin \DIFadd{on large scales }\DIFaddend as ratios of $P_\text{m,lin}(k)$, \DIFaddbegin \DIFadd{and on smaller scales via empirical fitting functions, }\DIFaddend which are obtained via arguments motivated by the halo model. These modifications are expressed as a series of equations with coefficients fitted to numerical simulations. Updated coefficients were introduced in \citet{Takahashi2012}. Note also that the normalisation of $P_\text{m,nl}(k)$ is ident\-ical to $P_\text{m,lin}(k)$, and is calculated using the linear form, since $\sigma_8$ refers to the linear power spectrum. The effect of introducing non-linearities is to increase small-scale power, with the recent coefficient updates slightly enhancing this effect.
...
Within each definition approach exists the potential for an infinite number of explicit definitions. The FOF approach has the linking length parameter (typically denoted $b$), whilst the SO approach has the overdensity threshold, \DIFdelbegin \DIFdel{$\Delta_h$}\DIFdelend \DIFaddbegin \DIFadd{$\Delta_{\rm h}$}\DIFaddend . Moreover, the conversion between FOF and SO masses is difficult because a linking length does not uniquely correspond to an overdensity \citep{More2011}.

\DIFaddbegin \DIFadd{Indeed, even conversion of relevant quantities (eg. mass functions, concentration-mass relations and bias functions) between different SO definitions is only ever approximate, due to practical difficulties in identifying halos in simulations. 
Accuracy of such conversions is discussed for concentration-mass relations in \mbox{%DIFAUXCMD
\citet{Diemer2015}}\hspace{0pt}%DIFAUXCMD
, who give an uncertainty of 15-20\% at high mass, and for mass functions in \mbox{%DIFAUXCMD
\citet{Bocquet2016}}\hspace{0pt}%DIFAUXCMD
, whose fits imply inaccuracies of 20-40\% below halo masses of $10^{15}M_\odot h^{-1}$.
}

\DIFaddend While it is thus difficult to compare, even in principle, between FOF and SO definitions, comparison of halo model quantities between explicit SO definitions is \DIFdelbegin \DIFdel{possible (to first order) }\DIFdelend \DIFaddbegin \DIFadd{approximately possible }\DIFaddend as long as one has a model for the density distribution within a halo (cf. \S\ref{sec:halomod:components:profile}).
...
While the shape of the filter may be arbitrary (and should be matched to the typical shapes of the primordial patches that give rise to the halos \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{Dalal2010, Chan2017,Diemer2019}}\hspace{0pt}%DIFAUXCMD
)}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{Dalal2010,Chan2017,Diemer2019}}\hspace{0pt}%DIFAUXCMD
...
%DIFDELCMD < %%%
\DIFdelend For an Einstein-de Sitter universe (in which the matter density equals the critical density and there is no dark energy), $\delta_c \approx 1.686$, and its value is very weakly dependent on cosmology\footnote{Note that even small changes in $\delta_c$ can affect the high-mass end of the HMF significantly. However, many authors assume $\delta_c = 1.686$ regardless of cosmology and thus many fits of the HMF are defined under this assumption.}
...
\begin{equation}
    \label{eq:fnu1}
    \int f(\nu) \DIFaddbegin {\rm \DIFaddend d\DIFaddbegin }\DIFaddend \nu = 1.
\end{equation}
...
Note also that from \DIFdelbegin \DIFdel{Eq. \ref{eq:theory:hmf_nu} }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cref{eq:theory:hmf_nu} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend we can determine $d\nu$ as a function of $dm$, and integrals of the form $\int n(m) g(m) dm$ may be re-written
\begin{equation}
    \label{eq:integrate_over_nu}
    \int g(\nu(m)) \frac{\rho_0}{m} f(\nu) \DIFaddbegin {\rm \DIFaddend d\DIFaddbegin }\DIFaddend \nu.
\end{equation}
In particular, the sum of all mass in the universe requires setting $g(m) = m$, and the requirement that the mass density in halos equal $\rho_0$ is easily seen to be equivalent to the condition \DIFdelbegin \DIFdel{Eq. 
\ref{eq:fnu1}. 
Integrating Eq. \ref{eq:integrate_over_nu} }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cref{eq:fnu1}}\hspace{0pt}%DIFAUXCMD
. 
Integrating \mbox{%DIFAUXCMD
\cref{eq:integrate_over_nu} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend can often be numerically advantageous, since relevant bounds on $f(\nu)$ can often be found independently of cosmology and redshift.

\DIFdelbegin \DIFdel{Figure \ref{fig:mfs} }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cref{fig:mfs} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend shows the HMF for \DIFdelbegin \DIFdel{four }\DIFdelend \DIFaddbegin \DIFadd{several }\DIFaddend of the fits available in \textsc{halomod} (via \textsc{hmf}). According to these fitting functions, the HMF behaves as a power-law at low masses, with an exponential cut-off above a typical mass $m_* \approx 5\times10^{14}$ (this mass changes with redshift). There is clearly significant variations between the fits, especially at high masses, for a range of reasons, including halo finder differences, alternate mass definitions, and simulation resolution \citep[cf.][]{Murray2013a,Knebe2011}. 

\begin{figure}
  \centering
  \includegraphics[width=\linewidth]{hmf_models.pdf}
  \caption[Several HMF fits]{\DIFdelbeginFL \DIFdelFL{Seven }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Several }\DIFaddendFL HMFs from the literature, including the `original' form of \protect\citet[][`PS']{Press1974}, motivated by spherical collapse, and \textsc{SMT} motivated by ellipsoidal collapse from \protect\citet{Sheth2001}\DIFaddbeginFL \DIFaddFL{. Note that the mass definitions and cosmologies assumed by the fits are rather disparate -- this is not meant to be an apples-to-apples comparison. While $\nu f(\nu)$ is almost independent of cosmology \mbox{%DIFAUXCMD
\citep{Jenkins2001}}\hspace{0pt}%DIFAUXCMD
, and in principle independent of mass definition (peak-height is based on the initial Lagrangian field}\DIFaddendFL )\DIFaddbeginFL \DIFaddFL{, in practice $f(\nu)$ is measured from simulations which must employ some halo-finding algorithm and mass definition}\DIFaddendFL .}
  \label{fig:mfs}
\end{figure}

%DIF < However, as seen in Figure \ref{fig:mf_fit_corr}, these variations are not as pronounced in the resulting $\xi_{gg}(r)$, \bd{Don't think we've even defined the galaxy-galaxy correlation?? Maybe this figure is premature at this point in the paper.} due to integration with the variation occurring at small values. The two-halo term is modified by a near-constant factor dependent primarily on the amplitude of $n(m_*)$. The one-halo term has a more complicated dependency. 
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  \begin{figure}
%DIF <    \centering
%DIF <    \includegraphics[width=\linewidth]{corr_mffits.pdf}
%DIF <    \caption[Effect of the HMF fit on the two-point correlation function]{Effect of the varying HMF fits on the two-point correlation function, where the mean galaxy density is matched in each case by shifting the HOD parameter $M_\text{min}$, where $M_\text{min} = 10^{11}h^{-1}M_\odot$ for the SMT function. The variation is reduced to $<10$\% for fits other than PS.}
%DIF <    \label{fig:mf_fit_corr}
%DIF <  \end{figure}
%DIFDELCMD < 
...
\end{equation}
which when combined with \DIFdelbegin \DIFdel{Eq. \ref{eq:hmf}}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cref{eq:hmf}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend , gives
\begin{equation}
    b(m) = 1 - \frac{1}{\delta_c} - \frac{\partial}{\partial \delta_c} \ln f(\nu).
\end{equation}
In this formalism, if all mass is contained within halos, according to \DIFdelbegin \DIFdel{Eq. \ref{eq:fnu1}}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cref{eq:fnu1}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend , then by construction, the following consistency relation holds,
\begin{equation}
...
\DIFaddbegin \DIFadd{Note that if Eq. \ref{eq:consistency} holds, the large-scale ($k\rightarrow0$) matter power is gauranteed to be equivalent to $P_m$ (which just says that the effective bias at large scales is unity).
}\DIFaddend 
...
In \DIFdelbegin \DIFdel{figure \ref{fig:bias_functions} }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cref{fig:bias_functions} }\hspace{0pt}%DIFAUXCMD
...
The scale-dependence of halo bias is a complex phenomenon, which has received a great deal of attention \citep[eg.][]{Paranjape2013,Lapi2014,Poole2015}. Foregoing these complexities, simple empirical models specify the scale-dependence as a function \DIFdelbegin \DIFdel{$S(\xi_m(r))$,
}\DIFdelend \DIFaddbegin \DIFadd{$S(\xi_{\rm m}(r))$,
}\DIFaddend \begin{equation}
    b^2(m,r) = b^2(m) S(\xi\DIFdelbegin \DIFdel{_m}\DIFdelend \DIFaddbegin \DIFadd{_{\rm m,nlin}}\DIFaddend (r)).
\end{equation}
An example form for $S$ is given by \citet{Tinker2005} as
\begin{equation}
	\label{eq:sdb}
	S(r) = \DIFdelbegin \DIFdel{\frac{\left[1+1.17\xi_m(r)\right]^{1.49}}{\left[1+0.69\xi_m(r)\right]^{2.09}}}\DIFdelend \DIFaddbegin \DIFadd{\frac{\left[1+1.17\xi_{\rm m,nlin}(r)\right]^{1.49}}{\left[1+0.69\xi_{\rm m,nlin}(r)\right]^{2.09}}}\DIFaddend .
\end{equation}
...
\DIFaddbegin \DIFadd{The correlation function used here is the non-linear matter correlation function from }\textsc{\DIFadd{halofit}}\DIFadd{.
}\DIFaddend 
...
While $\rho_s$ does not directly depend on mass in our formulation, the dependence on the mean density and the secondary dependence of $c$ on mass translate into a relation where more massive and earlier-forming halos have higher central densities. The $f(r/r_s)$ term controls the shape of the profile and is a function of the scaled radius $x=r/r_s$. Throughout the following discussion we shall often refer to a `reduced' form of the profile (or related quantities) in which simple normalisation factors are removed. An example of such a reduced form is $f$ in \DIFdelbegin \DIFdel{Eq.
\ref{eq:universalprofile}.
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cref{eq:universalprofile}}\hspace{0pt}%DIFAUXCMD
.
}\DIFaddend 

%DIF <  and at this stage only considers profiles with two parameters (one of which is set by the halo overdensity $\Delta_h$, and the other we label as ``concentration"). Multi-parameter profiles \citep[eg. the `Einasto' profile,][]{Navarro2004} will be left for future versions.
%DIF >  and at this stage only considers profiles with two parameters (one of which is set by the halo overdensity $\Delta_{\rm h}$, and the other we label as ``concentration"). Multi-parameter profiles \citep[eg. the `Einasto' profile,][]{Navarro2004} will be left for future versions.

\subsubsection{Truncated Halos}
For halos truncated at the halo radius, the amplitude of the profile is set by the total mass inside the halo radius, which we recover by integrating \DIFdelbegin \DIFdel{Eq. \ref{eq:universalprofile}:
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cref{eq:universalprofile}}\hspace{0pt}%DIFAUXCMD
:
}\DIFaddend \begin{equation}
\label{eq:minrv}
m = 4\pi \rho_s(c) r_s^3 \int_0^c x^2 f(x) \DIFdelbegin \DIFdel{dx}\DIFdelend \DIFaddbegin {\rm \DIFadd{d}}\DIFadd{x}\DIFaddend ,
\end{equation}
where we have made the substitution $r = xr_s$ in which \DIFdelbegin \DIFdel{$x(r_{\Delta_h}) = c$}\DIFdelend \DIFaddbegin \DIFadd{$x(r_{\Delta_{\rm h}}) = c$}\DIFaddend .
If we set
\begin{equation}
\label{eq:hofc}
h(c) = \int_0^c x^2 f(x) \DIFdelbegin \DIFdel{dx
}\DIFdelend \DIFaddbegin {\rm \DIFadd{d}}\DIFadd{x
}\DIFaddend \end{equation}
then we obtain
\begin{equation}
\label{eq:hofm}
\rho_s(c) = \frac{m}{4 \pi r_s^3 h(c)} \equiv \DIFdelbegin \DIFdel{\frac{c^3 \Delta_h}{3h(c)}}\DIFdelend \DIFaddbegin \DIFadd{\frac{c^3 \Delta_{\rm h}}{3h(c)}}\DIFaddend \bar{\rho}
\end{equation}
...
\end{equation}
Using \DIFdelbegin \DIFdel{Eq. \ref{eq:hofm} }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cref{eq:hofm} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend we have
\begin{equation}
\label{eq:uspherical}
...
\begin{equation}
\label{eq:lofxc}
l(x,c) = \int f(\mathbf{y},c)f(|\mathbf{y}+\mathbf{x}|,c)\DIFaddbegin {\rm \DIFaddend d\DIFaddbegin }\DIFaddend ^3y,
\end{equation}
...
In the case that the profile is not truncated at \DIFdelbegin \DIFdel{$r_{\Delta_h}$}\DIFdelend \DIFaddbegin \DIFadd{$r_{\Delta_{\rm h}}$}\DIFaddend , we still define \DIFdelbegin \DIFdel{$c \equiv r_{\Delta_h}/r_s$}\DIFdelend \DIFaddbegin \DIFadd{$c \equiv r_{\Delta_{\rm h}}/r_s$}\DIFaddend , and assume \DIFdelbegin \DIFdel{Eq. \ref{eq:mass-radius-halo} }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cref{eq:mass-radius-halo} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend relates halo mass to \DIFdelbegin \DIFdel{$r_{\Delta_h}$}\DIFdelend \DIFaddbegin \DIFadd{$r_{\Delta_{\rm h}}$}\DIFaddend . 
The integrated mass of the halo is defined by \DIFdelbegin \DIFdel{Eq. \ref{eq:minrv}}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cref{eq:minrv}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend , but the upper integration limit is set to $+\infty$. This integral must converge for the given profile in order for it to be well-defined as a non-truncated profile.
...
%\end{equation}
%DIF < which determines the concentration via Eq. \ref{eq:hofm}.
%DIF > which determines the concentration via \cref{eq:hofm}.
...
%DIF < \bd{I'm afraid I don't get this explanation at all. Since we sum the 1-halo and 2-halo terms, should they not both be present?} \sgm{Talk about this... essentially it means we don't double count.}
...
By far the more common approach is to set upper limits (say $m'_1$ and $m'_2$) on the 2-halo integral, \DIFdelbegin \DIFdel{Eq. \ref{eq:dmpower2a}}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cref{eq:dmpower2a}}\hspace{0pt}%DIFAUXCMD
...
It is well-known that halos are typically triaxial \citep{Bullock2001a,Taylor2011,Zemp2011}, and therefore one can improve the results by considering the probability that each pair is in different halos, given an empirical distribution of axis ratios. In this case, \DIFdelbegin \DIFdel{Eq. \ref{eq:dmpower2a} }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cref{eq:dmpower2a} }\hspace{0pt}%DIFAUXCMD
...
Setting $m_1' = m_2' = m'$, one can use the separated form,  \DIFdelbegin \DIFdel{Eq. \ref{eq:dmpower2}}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cref{eq:dmpower2}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend , to generate the final galaxy power spectrum.

We show the effect of several of these models in  \DIFdelbegin \DIFdel{figure \ref{fig:halo_exclusion}}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cref{fig:halo_exclusion}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . The spherical exclusion is the most effective at reducing pairs, with the $n_g$-matched method exhibiting a more gentle suppression. Interestingly, merely using the linear power without any exclusion is the closest to the $n_g$-matched case at the transition scales. 
...
For \DIFdelbegin \DIFdel{two-point statistics, an important quantity }\DIFdelend \DIFaddbegin \DIFadd{2-point statistics, important quantities }\DIFaddend arising from the HOD \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{are }\DIFaddend the mean pair counts \DIFdelbegin \DIFdel{, $\langle N_t(N_t-1) \rangle$}\DIFdelend \DIFaddbegin \DIFadd{within a halo, which we address separately for centrals and satellites}\DIFaddend .
We wish to derive \DIFdelbegin \DIFdel{this quantity in a form }\DIFdelend \DIFaddbegin \DIFadd{these quantities in forms }\DIFaddend explicitly dependent only on the mean occupation functions $\Nc$ and $\Ns$, for which we have specific parametrisations. 
\DIFdelbegin \DIFdel{We begin by noting, after \mbox{%DIFAUXCMD
\citet{Zheng2005}}\hspace{0pt}%DIFAUXCMD
, that
}\begin{displaymath}
    \DIFdel{\langle N_t(N_t-1) \rangle = \langle N_c(N_c-1) \rangle + \langle N_s(N_s-1) \rangle + \langle N_c N_s \rangle,
}\end{displaymath}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{Furthermore, we treat self-pairs separately from cross-pairs, as self-pairs have different correlation properties.
Noting that ultimately we will be comparing these pair counts to the mean number density squared (which counts each pair twice), we have:
}\begin{align}
    \DIFadd{N_{\rm cross-pair}^{\rm cen-cen} }&\DIFadd{= 0 }\\
    \DIFadd{N_{\rm cross-pair}^{\rm cen-sat} }&\DIFadd{= 2 \langle N_c N_s \rangle }\\
    \DIFadd{N_{\rm cross-pair}^{\rm sat-sat} }&\DIFadd{= \langle N_s (N_s -1) \rangle \langle N_s \rangle^2
}\end{align}\DIFaddend 
\DIFdelbegin \DIFdel{in which the first term on the right vanishes since $N_c$ is either $0$ or $1$, and }\DIFdelend \DIFaddbegin \DIFadd{where the last equality is made }\DIFaddend under the assumption \DIFdelbegin \DIFdel{that $N_s$ has a Poisson distribution, we can simplify to
}\begin{displaymath}
    \DIFdel{\label{eq:nt_pairs}
    \langle N_t(N_t-1) \rangle = \Ns^2 + \langle N_c N_s \rangle.
}\end{displaymath}%DIFAUXCMD
\DIFdel{The last }\DIFdelend \DIFaddbegin \DIFadd{of Poisson-distributed satellite counts.
%DIF >  We begin by noting, after \citet{Zheng2005}, that
%DIF >  \begin{equation}
%DIF >      \langle N_{\rm t}(N_{\rm t}-1) \rangle = \langle N_{\rm c}(N_{\rm c}-1) \rangle + \langle N_{\rm s}(N_{\rm s}-1) \rangle + 2\langle N_{\rm c} N_{\rm s} \rangle,
%DIF >  \end{equation}
%DIF >  in which the first term on the right vanishes since $N_{\rm c}$ is either $0$ or $1$, and under the assumption that $N_{\rm s}$ has a Poisson distribution, we can simplify to
%DIF >  \begin{equation}
%DIF >      \label{eq:nt_pairs}
%DIF >      \langle N_{\rm t}(N_{\rm t}-1) \rangle = \Ns^2 + 2\langle N_{\rm c} N_{\rm s} \rangle.
%DIF >  \end{equation}
The cen-sat }\DIFaddend term is problematic \DIFaddbegin \DIFadd{(as it is not a function purely of mean counts of either central or satellite)}\DIFaddend , however if we impose the central condition, then \DIFdelbegin \DIFdel{$\langle N_c N_s \rangle = \Ns$}\DIFdelend \DIFaddbegin \DIFadd{$\langle N_{\rm c} N_{\rm s} \rangle = \Ns$}\DIFaddend , since we need only count halos in which \DIFdelbegin \DIFdel{$N_c = 1$. 
Thus}\DIFdelend \DIFaddbegin \DIFadd{$N_{\rm c} = 1$. 
%DIF > Thus, if the central condition holds, we achieve the final form
%DIF > \begin{equation}
%DIF >     \langle N_{\rm t}(N_{\rm t}-1) \rangle = \Ns(2 + \Ns).
%DIF > \end{equation}
Conversely}\DIFaddend , if the \DIFdelbegin \DIFdel{central condition holds, we achieve the final form
}\begin{displaymath}
    \DIFdel{\langle N_t(N_t-1) \rangle = \Ns(1 + \Ns).
}\end{displaymath}%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{If the }\DIFdelend parameterisations for $\Nc$ and $\Ns$ do not impose the central condition then we can approximate, following \citet{Zehavi2005}, with \DIFdelbegin \DIFdel{$\langle N_c N_s \rangle = \Nc \Ns$}\DIFdelend \DIFaddbegin \DIFadd{$\langle N_{\rm c} N_{\rm s} \rangle = \Nc \Ns$}\DIFaddend , given that $\Ns \ll 1$ when $\Nc < 1$. For $\Nc = 0$ or 1, it is no longer an approximation, and thus for step-function parameterisations of $\Nc$, it is completely accurate (for arbitrary parameterisations of $\Ns$).
...
\DIFaddbegin \DIFadd{Clearly, the number of self-pairs is simply $\langle N_t \rangle$.
}

\DIFaddend There are several parameterisations for the HOD in the literature. In this work, and correspondingly in the \verb|halomod| framework, we focus on those that utilise the separation of
...
\begin{figure}
\centering
\includegraphics[width=\linewidth]{hod_models.pdf} 
...
The extension of equations \ref{eq:dmcorr1}, \ref{eq:dmpower1} and \ref{eq:dmpower2} to include galaxies is relatively simple under the assumption that all galaxies reside in halos, and their intra-halo abundance follows that of the density profile \DIFaddbegin \DIFadd{for satellites}\DIFaddend . 

\DIFdelbegin \DIFdel{The primary modification of }\DIFdelend \DIFaddbegin \DIFadd{For the 2-halo term, we make a simple modification to }\DIFaddend the DM equations\DIFdelbegin \DIFdel{is to replace }\DIFdelend \DIFaddbegin \DIFadd{, replacing }\DIFaddend factors of $m/\bar{\rho}$ with \DIFdelbegin \DIFdel{$N_t(m)/\bar{n}_g$, where $N_t(m)$ }\DIFdelend \DIFaddbegin \DIFadd{$N_{\rm t}(m)/\bar{n}_g$, where $N_{\rm t}(m)$ }\DIFaddend is the mean occupation function from the HOD (where for simplicity we omit angle brackets for the rest of this work), and $\bar{n}_g$ is the mean number density of galaxies,
\begin{equation}
    \label{eq:meandens}
 \bar{n}_g = \int n(m)N\DIFdelbegin \DIFdel{_t}\DIFdelend \DIFaddbegin \DIFadd{_{\rm t}}\DIFaddend (m) \DIFdelbegin \DIFdel{dm}\DIFdelend \DIFaddbegin {\rm \DIFadd{d}}\DIFadd{m}\DIFaddend .
\end{equation}
With this modification the 2-halo term is simply described by
\begin{equation}
    \DIFaddbegin \label{eq:Ig}
    \DIFaddend I\DIFdelbegin \DIFdel{_g}\DIFdelend \DIFaddbegin \DIFadd{_{\rm g}}\DIFaddend (k,m) = n(m) u(k|m) N\DIFdelbegin \DIFdel{_t}\DIFdelend \DIFaddbegin \DIFadd{_{\rm t}}\DIFaddend (m)/\bar{n}_g,
\end{equation}
which serves as a direct replacement for $I_\text{DM}$ in equations \ref{eq:dmpower2a} and \ref{eq:dmpower2}.
\DIFaddbegin \DIFadd{Note that this is not }\textit{\DIFadd{quite}} \DIFadd{accurate given the separation of galaxies into centrals and satellites, because in this case the central galaxy is constrained to be at the center instead of following the profile (whereas Eq. \ref{eq:Ig} assumes all of the galaxies---including the central---are located following the density profile). Nevertheless, the error induced by this approximation is extremely small.
Note further that no ``self-pairs'' are included in the 2-halo term, since it always considers two separate halos (thus it contains no ``shot-noise'').
}\DIFaddend 


The 1-halo term needs a little more care. 
\DIFdelbegin \DIFdel{The primary modification gives
}\begin{displaymath}
\DIFdel{%DIFDELCMD < \label{eq:galcoor1}%%%
1 + \xi_g^{1h}(r) = \frac{1}{\bar{n}_g^2}\int n(m) \frac{\langle N_t(N_t-1) \rangle}{2} \lambda(r|m) dm,
}\end{displaymath}%DIFAUXCMD
\DIFdel{however using }\DIFdelend \DIFaddbegin \DIFadd{Using }\DIFaddend the formalism of \S\ref{sec:theory-gal:hod}, we \DIFdelbegin \DIFdel{can }\DIFdelend separate the pair counts into contributions from central-satellite ($c-s$) \DIFdelbegin \DIFdel{pairs, }\DIFdelend and satellite-satellite ($s-s$) \DIFdelbegin \DIFdel{pairs (cf. Eq. \ref{eq:nt_pairs}}\DIFdelend \DIFaddbegin \DIFadd{cross-pairs (we shall deal with self-pairs shortly}\DIFaddend ). 
The first of these is a one-point quantity dependent \DIFdelbegin \DIFdel{only on the profile, rather than the }\DIFdelend \DIFaddbegin \DIFadd{on the density profile, while the latter is given by the }\DIFaddend self-convolution \DIFaddbegin \DIFadd{of the profile (similar to DM)}\DIFaddend . 
Thus we have
\DIFdelbegin \DIFdel{, for the correlations
}\DIFdelend \begin{equation}
\DIFaddbegin \label{eq:1-halo-galaxy}
\DIFaddend 1 + \xi\DIFdelbegin \DIFdel{_g^{1h}}\DIFdelend \DIFaddbegin \DIFadd{_{\rm gg}^{\rm 1h}}\DIFaddend (r) = \DIFdelbegin \DIFdel{\frac{2}{\bar{n}_g^2}}\DIFdelend \DIFaddbegin \DIFadd{\frac{1}{\bar{n}_g^2}}\DIFaddend \int n(m) \left[\DIFaddbegin \DIFadd{2}\DIFaddend \langle N\DIFdelbegin \DIFdel{_c }\DIFdelend \DIFaddbegin \DIFadd{_{\rm c} }\DIFaddend N\DIFdelbegin \DIFdel{_s }\DIFdelend \DIFaddbegin \DIFadd{_{\rm s} }\DIFaddend \rangle \DIFdelbegin \DIFdel{\rho(r|m) }\DIFdelend \DIFaddbegin \DIFadd{\frac{\rho(r|m)}{m} }\DIFaddend + N\DIFdelbegin \DIFdel{_s}\DIFdelend \DIFaddbegin \DIFadd{_{\rm s}}\DIFaddend ^2 \DIFdelbegin \DIFdel{\lambda(r|m)}\DIFdelend \DIFaddbegin \DIFadd{\frac{\lambda(r|m)}{m^2}}\DIFaddend \right] \DIFdelbegin \DIFdel{dm}\DIFdelend \DIFaddbegin {\rm \DIFadd{d}}\DIFadd{m}\DIFaddend ,
\end{equation}
where the quantity \DIFdelbegin \DIFdel{$\langle N_c N_s \rangle$ }\DIFdelend \DIFaddbegin \DIFadd{$\langle N_{\rm c} N_{\rm s} \rangle$ }\DIFaddend is treated differently depending on whether the central condition is imposed (cf. \S\ref{sec:theory-gal:hod}).
\DIFdelbegin \DIFdel{The Fourier-space counterpart is defined likewise.}\DIFdelend \DIFaddbegin 

\DIFadd{The behaviour of this expression as the separation approaches zero is of some interest. We firstly note that as $r$ falls below some (very small) threshold, we should no longer trust the HOD model -- some level of ``galaxy exclusion'' must apply \mbox{%DIFAUXCMD
\citep{Ginzburg2017}}\hspace{0pt}%DIFAUXCMD
, rendering the the distribution of $N_s$ non-Poisson and non-independent of the surrounding locations. Thus Eq. \ref{eq:1-halo-galaxy} will tend to over-estimate the correlation function below this threshold. This is most evident at $r=0$, where Eq. \ref{eq:1-halo-galaxy} predicts a non-zero correlation, even though we have explicitly avoided self-pairs. 
}\DIFaddend 

\DIFaddbegin \DIFadd{On the other hand, self-pairs are really there. They form a delta-function contribution at $r=0$, with a magnitude given by the mean galaxy density}\footnote{\DIFadd{Note that for non-discrete tracers, which }\halomod\DIFadd{\ supports, the `shot-noise' contribution is not simply given by the tracer number density, but requires knowledge of the scatter in the tracer HOD population (which is no longer Poisson, cf. \mbox{%DIFAUXCMD
\citealt{Chen2021}}\hspace{0pt}%DIFAUXCMD
).}}\DIFadd{. 
This translates into a constant with magnitude $1/\bar{n}_g$ in the power spectrum, and this is properly the `shot-noise'. 
In }\textsc{\DIFadd{halomod}}\DIFadd{, the power spectra are defined without shot-noise (i.e. purely as the FT of Eq. \ref{eq:1-halo-galaxy}), which can be added in by the user if desired simply as a constant. The power spectrum is thus
}\begin{equation}
    \DIFadd{P_{\rm gg}^{\rm 1h}(k) = \int }{\DIFadd{\rm d}}\DIFadd{m\ n(m)u(k|m) \left[N_s^2 u(k|m) + 2 \langle N_s N_c \rangle \right].
}\end{equation}
\DIFadd{Note again that the }\texttt{\DIFadd{force\_1halo\_turnover}} \DIFadd{option forces the 1-halo term to turnover on scales larger than the size of halos (cf. \S\ref{sec:halomod:frameworks:base}), to reduce the spurious `shot-noise', and the true shot-noise can be manually added back in to the power spectrum.
}



\DIFaddend \subsection{Derived Quantities}
...
\begin{equation}
 P\DIFdelbegin \DIFdel{_{gg}}\DIFdelend \DIFaddbegin \DIFadd{_{\rm gg}}\DIFaddend (k) \approx b^2_\text{eff}P\DIFdelbegin \DIFdel{_m}\DIFdelend \DIFaddbegin \DIFadd{_{\rm m}}\DIFaddend (k),
\end{equation}
where
\begin{equation}
\label{eq:beff}
 b_\text{eff} = \frac{1}{\bar{n}_g}\int n(m)b(m)N\DIFdelbegin \DIFdel{_t}\DIFdelend \DIFaddbegin \DIFadd{_{\rm t}}\DIFaddend (m) \DIFdelbegin \DIFdel{dm
}\DIFdelend \DIFaddbegin {\rm \DIFadd{d}}\DIFadd{m
}\DIFaddend \end{equation}
...
\textsc{Halomod} is a \DIFdelbegin \DIFdel{Python }\DIFdelend \DIFaddbegin \python \DIFaddend package, built on the \textsc{hmf} package\footnote{\DIFdelbegin %DIFDELCMD < \url{https://github.com/steven-murray/hmf}%%%
...
\begin{figure}
    \centering
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\linewidth,trim={0 0.2cm 0 0},clip=true]{toy_example.pdf}
%DIFDELCMD <     %%%
...
We also provide extensive interoperability with the \textsc{\DIFdelbegin \DIFdel{Colossus}\DIFdelend \DIFaddbegin \DIFadd{colossus}\DIFaddend } code\footnote{\url{http://www.benediktdiemer.com/code/colossus/}} \citep{Diemer2018}, by providing component constructors for models contained in \textsc{\DIFdelbegin \DIFdel{Colossus}\DIFdelend \DIFaddbegin \DIFadd{colossus}\DIFaddend }. 
...
\end{lstlisting}
and get the full range of functionality from the \textsc{\DIFdelbegin \DIFdel{Colossus}\DIFdelend \DIFaddbegin \DIFadd{colossus}\DIFaddend }-defined bias model within the \textsc{halomod} framework (even setting of model-dependent parameters).
At the time of publishing, these constructors are defined for bias and halo concentration models, as these models are part of the core functionality of \textsc{\DIFdelbegin \DIFdel{Colossus}\DIFdelend \DIFaddbegin \DIFadd{colossus}\DIFaddend } and thus there is a real chance that the models may exist in \textsc{\DIFdelbegin \DIFdel{Colossus}\DIFdelend \DIFaddbegin \DIFadd{colossus}\DIFaddend } and not \textsc{halomod}. 
...
In this section we present \DIFdelbegin \DIFdel{two }\DIFdelend \DIFaddbegin \DIFadd{several }\DIFaddend of the frameworks that \textsc{halomod} provides. We note that several other frameworks are found in the \textsc{hmf} package, from which those presented here inherit. Furthermore, \textsc{halomod} also provides a \verb|DMHaloModel| framework for calculating non-linear matter power spectra and correlation functions \DIFdelbegin \DIFdel{, and a }\texttt{\DIFdel{Cross\-Correlations}} %DIFAUXCMD
\DIFdel{framework for cross-correlating two tracers. We do not present these here explicitly, but note that the }%
{\color{red}%DIFAUXCMD
\verb|DMHaloModel| %DIFAUXCMD
}%DIFAUXCMD
%DIFDELCMD < \verb|DMHaloModel| %%%
\DIFdel{is a strict subset of the }%
{\color{red}%DIFAUXCMD
\verb|TracerHaloModel| %DIFAUXCMD
}%DIFAUXCMD
%DIFDELCMD < \verb|TracerHaloModel| %%%
\DIFdel{which we do present. }\DIFdelend \DIFaddbegin \DIFadd{(the same are available within the }\texttt{\DIFadd{TracerHaloModel}}\DIFadd{, but the }\texttt{\DIFadd{DMHaloModel}} \DIFadd{requires fewer inputs). %DIF > We do not present these here explicitly, but note that the {\color{blue}%DIFAUXCMD
\verb|DMHaloModel| %
}%DIFAUXCMD
is a strict subset of the {\color{blue}%DIFAUXCMD
\verb|TracerHaloModel| %
}%DIFAUXCMD
which we do present.
}\DIFaddend 
...
\texttt{takahashi} & True & Apply updated \textsc{halofit} coefficients from \citet{Takahashi2012}\DIFaddbeginFL \DIFaddFL{, otherwise use original coefficiencts from \mbox{%DIFAUXCMD
\citet{Smith2003}}\hspace{0pt}%DIFAUXCMD
. }\DIFaddendFL \\
\texttt{hc\_spectrum} & \DIFdelbeginFL \DIFdelFL{Nonlinear }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Linear }\DIFaddendFL & Model for \DIFdelbeginFL \DIFdelFL{$P_{hh}^c(k)$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$P_{\rm hh}^c(k)$}\DIFaddendFL . Either ``linear", ``nonlinear" (\textsc{halofit}), or ``filtered-nl" (smoothed on a scale of 2$h^{-1}$Mpc). \\
\texttt{force\_1halo\_turnover} & True & Whether to cut off the 1-halo power on extremely large scales.  \\
\DIFaddbeginFL \texttt{\DIFaddFL{force\_unity\_dm\_bias}} & \DIFaddFL{True }& \DIFaddFL{Whether to renormalize the 2-halo term such that the DM bias is unity for $k\rightarrow0$.  }\\
\DIFaddendFL 
...
Length-$k$ & $T$, \DIFdelbeginFL \DIFdelFL{$P_m$, $P_{m, \text{halofit}}$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$P_{\rm m}$, $P_{{\rm m}, \text{halofit}}$}\DIFaddendFL , $\Delta^2$, $\Delta^2_\text{halofit}$, \DIFdelbeginFL \DIFdelFL{$P_{gg}^{ss}$, $P_{gg}^{2h}$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$P_{\rm gg}^{ss}$, $P_{\rm gg}^{\rm 2h}$}\DIFaddendFL , $P_{mm}$, \DIFdelbeginFL \DIFdelFL{$P_{mm}^{1h}$, $P_{mm}^{2h}$, $P_{mg}^{1h}$, $P_{mg}^{2h}$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$P_{mm}^{\rm 1h}$, $P_{mm}^{\rm 2h}$, $P_{mg}^{\rm 1h}$, $P_{mg}^{\rm 2h}$}\DIFaddendFL , $P_{mg}$ \\
Length-$r$ & \DIFdelbeginFL \DIFdelFL{$\xi_{gg}^{1h}$, $\xi_{gg}^{cs}$, $\xi_{gg}^{ss}$, $\xi_{gg}^{2h}$, $\xi_{gg}$, $\xi_{DM}^{1h}$, $\xi_{DM}^{2h}$, $\xi_{DM}$, $\xi_m$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\xi_{\rm gg}^{\rm 1h}$, $\xi_{\rm gg}^{cs}$, $\xi_{\rm gg}^{ss}$, $\xi_{\rm gg}^{\rm 2h}$, $\xi_{\rm gg}$, $\xi_{\rm DM}^{\rm 1h}$, $\xi_{\rm DM}^{\rm 2h}$, $\xi_{\rm DM}$, $\xi_{\rm m}$}\DIFaddendFL , $\xi_{m, \text{halofit}}$, \DIFdelbeginFL \DIFdelFL{$\xi_{mg}^{1h}$, $\xi_{mg}^{2h}$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\xi_{mg}^{\rm 1h}$, $\xi_{mg}^{\rm 2h}$}\DIFaddendFL , $\xi_{mg}$, \DIFdelbeginFL \DIFdelFL{$S(\xi_m(r))$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$S(\xi_{\rm m}(r))$ }\DIFaddendFL \\
...
At large scales, the 1-halo power spectrum converges to a non-zero constant (this is clear from \DIFdelbegin \DIFdel{Eq. \ref{eq:dmpower1}}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cref{eq:dmpower1}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend , since $u$ asymptotes to unity as $k\rightarrow0$). This is generally considered to be the ``shot-noise'' term, and is clearly unphysical when considering a smooth distribution. 
\verb|TracerHaloModel| offers to force the 1-halo term to turnover on scales larger than about 10 halo radii. 

Explicitly, it changes the lower mass limit of the integration in \DIFdelbegin \DIFdel{Eq. \ref{eq:dmpower1} to be
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cref{eq:dmpower1} }\hspace{0pt}%DIFAUXCMD
to be
}\DIFaddend \begin{equation}
    m_{\rm lim}(k) = \frac{4\pi}{3} \left(\frac{\pi}{10k}\right)^3 \bar{\rho}_0 \Delta\DIFdelbegin \DIFdel{_h}\DIFdelend \DIFaddbegin \DIFadd{_{\rm h}}\DIFaddend .
\end{equation}

\DIFaddbegin \paragraph*{\DIFadd{Large-scale matter-matter bias}}
\DIFadd{The large-scale matter power under the halo model ansatz (i.e. ``all mass is inside halos'') is by definition equivalent to the linear power. 
Another way of saying this is that the large-scale effective bias, Eq. \ref{eq:beff}, for matter is unity. This, in principle, is true if the consistency relation, Eq. \ref{eq:consistency}, holds. However, there are several ways in which this condition will fail -- either in principle or in practice. 
}

\DIFadd{The first is that in some halo models, the ansatz itself is specifically broken. 
For example, halo models of warm dark matter may invoke a ``smooth'' dark matter 
component that is not collapsed into halos \mbox{%DIFAUXCMD
\citep[eg.][]{Smith2011a}}\hspace{0pt}%DIFAUXCMD
. In this case,
the HMF model is specifically tuned to not integrate to the mean density.
}

\DIFadd{The second is when a bias or HMF relation is chosen such that Eq. \ref{eq:consistency} is not satisfied. Sometimes these relations may provide better
accuracy at large masses, at the expense of being incompatible with the halo model ansatz. Typically, in these cases, one still }\textit{\DIFadd{desires}} \DIFadd{the behaviour that the matter power converge to the linear power at large scales. The best way to avoid this situation is to avoid choosing incompatible relations when computing matter statistics (such relations are usually fine for tracer statistics, as these generally trace only halos above some minimum mass).
}

\DIFadd{The third is simply due to the inherent limitation of the numerical integration. Even with compatible HMF and bias relations, the numerical integration to compute $b(k, r)$ is over a finite range of mass, and thus will not necessarily converge to unity at large scales. This is always undesirable. 
}

\DIFadd{To deal with these issues, }\textsc{\DIFadd{halomod}} \DIFadd{includes an option }\texttt{\DIFadd{force\_unity\_dm\_bias}}\DIFadd{, which if set, manually sets the effective bias of matter to unity, and then renormalizes the 2-halo term such that it approaches these effective bias at large scales. It does this irrespective of the HMF and bias relation chosen, which is generally the desired behaviour.
}


\DIFaddend \subsubsection{Projected correlation function}
\label{sec:projcorr}
...
%DIF <  \texttt{proj\_limit} & Optional upper limit on integral (Eq. \ref{eq:wprp}) \\
%DIF >  \texttt{proj\_limit} & Optional upper limit on integral (\cref{eq:wprp}) \\
% \midrule
...
\begin{equation}
	\label{eq:wprp}
	w_p(r_p) = 2\int_{r_p}^\infty \DIFdelbegin \DIFdel{\frac{r\xi(r)}{\sqrt{r^2-r_p^2}}dr}\DIFdelend \DIFaddbegin \DIFadd{\frac{r\xi(r)}{\sqrt{r^2-r_p^2}}{\rm d}r}\DIFaddend .
\end{equation}
...
\DIFaddbegin \DIFadd{Angular correlation functions can be computed using the }{\color{blue}%DIFAUXCMD
\verb|AngularCF| %
}%DIFAUXCMD
\DIFadd{class (which again inherits from }\texttt{\DIFadd{TracerHaloModel}}\DIFadd{). 
}

\DIFaddend The angular correlation function between two density fields (potentially the same field in an auto-correlation) is defined as \citep{Simon2007}
\begin{equation}
    w(\theta) \approx \int_0^\infty \DIFdelbegin \DIFdel{dr}\DIFdelend \DIFaddbegin {\rm \DIFadd{d}}\DIFadd{r}\DIFaddend _1\int_0^\infty \DIFdelbegin \DIFdel{dr}\DIFdelend \DIFaddbegin {\rm \DIFadd{d}}\DIFadd{r}\DIFaddend _2\ p_1(r_1) p_2(r_2) \xi(R,\bar{r}),
\end{equation} 
...
\begin{equation}
   w(\theta) = 2\int_0^\infty \DIFaddbegin {\rm \DIFaddend d\DIFaddbegin }\DIFaddend \bar{r} p_1(\bar{r}) p_2(\bar{r}) \int_0^{\infty} \DIFaddbegin {\rm \DIFaddend d\DIFaddbegin }\DIFaddend \Delta r \xi(R,\bar{r}), 
\end{equation}
where 
\begin{equation}
...
\DIFaddbegin \subsubsection{\DIFadd{Tracer-Tracer Cross-Correlations}}
\label{sec:halomod:frameworks:crosscorr}
\DIFadd{While the }\texttt{\DIFadd{TracerHaloModel}} \DIFadd{class is able to compute cross-correlations between matter and a tracer field, it is often useful to be able to compute cross-correlations between two tracer fields (eg. optical galaxies and neutral hydrogen, or two classes of optical galaxies). This functionality is supported via the }\texttt{\DIFadd{CrossCorrelations}} \DIFadd{class, to which the user provides two }\texttt{\DIFadd{TracerHaloModel}} \DIFadd{instances. Note that currently, halo exclusion is not supported in cross-correlations.
}

\DIFadd{In addition to defining the properties of the individual tracer populations, one must define the way the two populations correlate within halos (this is immediately clear when considering two overlapping populations of galaxies, for example). Within }\halomod\DIFadd{, we use the following relations:
}\begin{equation}
    \DIFadd{\langle T^{c,s}_{1} T^{c,s}_2 \rangle = \langle T^{c,s}_1 \rangle \langle T^{c,s}_2 \rangle + \sigma^{c,s}_1 \sigma^{c,s}_1 R^{c-s, s-c, s-s}_{12} - \delta_{ss}Q_{ss},
}\end{equation}
\DIFadd{where $c$ and $s$ stand for central and satellite, and may be the same or different for each of the populations (i.e. we have central-satellite, satellite-central and satellite-satellite terms, but not a central-central term since the central tracer, if it exists, must be identical between the populations).
The final term, which is only non-zero in the satellite-satellite case, is in that case the average occupation of tracers shared between the population per halo of mass $m$. 
This relation is nothing more than the standard definition of the variance of a product of random variables, in which the $\sigma$ is the standard deviation in the occupation for each population, and $R$ is a correlation coefficient between -1 and 1. 
For independent/uncorrelated populations, $R=0$.
For an autocorrelation, $R=1$ and $Q=N_s$, and we obtain the same expressions presented in \S\ref{sec:theory-gal:hod}.
More details and a worked example in the context of cross-correlations of optical galaxies and }\textsc{\DIFadd{Hi}} \DIFadd{is provided in \mbox{%DIFAUXCMD
\citet{Wolz2019}}\hspace{0pt}%DIFAUXCMD
.
}

\DIFadd{In }\halomod\DIFadd{, we allow the user to specify the variance of the occupation (for both centrals and satellites) within the }\texttt{\DIFadd{HOD}} \DIFadd{component (cf. \S\ref{sec:halomod:components:hod}). We additionally provide a }\texttt{\DIFadd{HODCross}} \DIFadd{component that allows combining two HOD models, and allows specifying $R$ for the three available combinations of centrals and satellites.
}


\DIFaddend \subsubsection{Warm dark matter models}
...
The various transfer models included in \textsc{hmf} are summarized in Table \ref{tab:models_transfer}. A comparison of the models with default parameters is shown in \DIFdelbegin \DIFdel{figure \ref{fig:power}}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cref{fig:power}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .

% \begin{figure}
%   \centering
%   \includegraphics[width=\linewidth]{transfer_fits.pdf}
...
\citet{Bardeen1986} & \texttt{BBKS} & $\displaystyle \frac{\ln(1+aq)}{aq} \left[\sum_{i=0}^4 (c_i q)^i\right]^{-1/4}$ & \DIFdelbeginFL \DIFdelFL{$\displaystyle q=\frac{k\exp(\Omega_b(1 + \sqrt{2h}/\Omega_m))}{\Gamma}$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\displaystyle q=\frac{k\exp[\Omega_b(1 + \sqrt{2h}/\Omega_m)]}{\Gamma h {\rm Mpc}^{-1} }$}\DIFaddendFL , $\Gamma = \Omega_mh$,  $a=2.34$, $c=$(1, 3.89, 16.1, \DIFdelbeginFL \DIFdelFL{5.47}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{5.46}\DIFaddendFL , 6.71) \\
...
\bottomrule[0.05cm]
\end{tabu}}
\caption{Summary of included \texttt{Transfer} models. \DIFaddbeginFL \DIFaddFL{Note that the $\Omega_b$-dependent exponential factor in the BBKS model of $q$ is drawn from \mbox{%DIFAUXCMD
\citet{Sugiyama1995} }\hspace{0pt}%DIFAUXCMD
and \mbox{%DIFAUXCMD
\citet{Liddle2000}}\hspace{0pt}%DIFAUXCMD
, and is optional within }\halomod\DIFaddFL{. Note also that these equations assume $k$ to be in units of $h/{\rm Mpc}$, which explains why $q$ is divided by $h$ instead of $h^2$.}\DIFaddendFL }
...
\cref{eq:massvariance}}\hspace{0pt}%DIFAUXCMD
. 
}\DIFaddend 
...
Table \ref{tab:models_window} summarises the window functions included in \textsc{halomod}, while \DIFdelbegin \DIFdel{figure \ref{fig:filter_sigma} }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cref{fig:filter_sigma} }\hspace{0pt}%DIFAUXCMD
...
%DIF <  \multirow{4}{*}{\textbf{Extra}} & \texttt{dlnss\_dlnr(r)} &  $\frac{d \ln \sigma^2(r)}{d\ln r}$, Eq. \ref{eq:dlnssdlnr} \\
%DIF <  & \texttt{dlnss\_dlnm(r)} & $\frac{d \ln \sigma^2(r)}{d\ln m}$, Eq. \ref{eq:dlnssdlnm} \\
%DIF >  \multirow{4}{*}{\textbf{Extra}} & \texttt{dlnss\_dlnr(r)} &  $\frac{d \ln \sigma^2(r)}{d\ln r}$, \cref{eq:dlnssdlnr} \\
%DIF >  & \texttt{dlnss\_dlnm(r)} & $\frac{d \ln \sigma^2(r)}{d\ln m}$, \cref{eq:dlnssdlnm} \\
...
\multirow{4}{*}{\texttt{TopHat}}  & $\displaystyle W(r,R) = H(R-r)$ \\
 ...
 & \DIFdelbeginFL \DIFdelFL{$\displaystyle \frac{d W}{d \ln x} = \frac{9  x \cos x + 3 (x^2 - 3)  \sin x) }{ x ^3}$  }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\displaystyle \frac{{\rm d} W}{{\rm d} \ln x} = \frac{9  x \cos x + 3 (x^2 - 3)  \sin x) }{ x ^3}$  }\DIFaddendFL \\
\midrule
...
\multirow{5}{*}{\texttt{SharpK}}  & $\displaystyle W(r,R) = \frac{\sin(r/R) - (r/R)\cos(r/R)}{2\pi^2 r^3}$   \\
 ...
 & \DIFdelbeginFL \DIFdelFL{$\displaystyle \frac{d W}{d \ln x} = \delta_D(x-1)$  }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\displaystyle \frac{{\rm d} W}{{\rm d} \ln x} = \delta_D(x-1)$  }\DIFaddendFL \\
 & \DIFdelbeginFL \DIFdelFL{$\displaystyle \frac{d \ln \sigma^2(r)}{d\ln r} = -\frac{P(1/r)}{2\pi^2 \sigma^2(r) r^3} $  }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\displaystyle \frac{{\rm d} \ln \sigma^2(r)}{{\rm d}\ln r} = -\frac{P(1/r)}{2\pi^2 \sigma^2(r) r^3} $  }\DIFaddendFL \\
 \midrule
...
\multirow{4}{*}{\texttt{Gaussian}}  & $\displaystyle W(r,R) = \frac{\exp(-r^2/2R^2)}{(2\pi)^{3/2} R^3}$  \\
 ...
 & \DIFdelbeginFL \DIFdelFL{$\displaystyle \frac{d W}{d \ln x} = -xW(x)$  }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\displaystyle \frac{{\rm d} W}{{\rm d} \ln x} = -xW(x)$  }\DIFaddendFL \\
...
\DIFaddbegin \DIFadd{Each HMF model has a class attribute }\texttt{\DIFadd{normalized}}\DIFadd{, which specifies whether the particular HMF integrates to the mean density (i.e. all mass is in halos). Note that this is an }\textit{\DIFadd{in principle}} \DIFadd{attribute -- when actually integrating the HMF, a limited mass range is used, and there is no guarantee that any particular model (even if normalized in principle) will integrate to the mean density.
}\DIFaddend %To enhance consistency, wherever possible, we reformulate the functions in terms of $\nu$, which is required by the peak-background split arguments to generate a bias function. In cases where the original formulation was in terms of $\sigma$, the quoted parameters will be modified. Of course, in the original, a change in $\delta_c$ (which is often neglected, but happens with changes in cosmology) would not affect the fit at all, whereas in the new it will. The extent of this effect will be very small, and it is unlikely in most applications that $\delta_c$ will modified from its default value of 1.686.
%
...
\DIFaddbegin \DIFadd{The mass function is dependent on the halo mass definition.
For a given halo mass, a higher overdensity criterion means that fewer peaks will exist. 
This effectively changes $f(\nu)$, since all other factors in Eq. \ref{eq:theory:hmf_nu} remain constant with mass definition (as they depend on the Lagrangian mass). 
The precise form of this dependence is extremely complicated; 
for one, changing mass definition does not necessarily evolve the masses of individual halos smoothly -- there is considerable scatter in the distribution of profiles for halos of a given mass, and for high masses where the mass function is extremely sensitive to mass, this can result in Eddington bias.
Furthermore, the under the halo model ansatz, all mass is contained in halos, so by contracting a halo to a new overdensity, one does not merely change the mass of the given halo, but also necessarily creates several new lower-mass halos that were previously subsumed in the larger halo. 
Thus, converting a mass function from one definition to another is fraught with peril.
}

\DIFadd{Nevertheless, there is a reasonably simple approximate conversion that can be made. For example, \mbox{%DIFAUXCMD
\citet{Bocquet2016} }\hspace{0pt}%DIFAUXCMD
uses the chain rule to give the relation
}\begin{equation}
    \DIFadd{\frac{dn}{dm'} = \frac{dn}{dm} \frac{dm}{dm'} = f(\nu) \frac{\rho_0}{m'} \frac{{\rm d}\nu}{{\rm d}m'}\times \frac{m'}{m},
}\end{equation}
\DIFadd{where the modification to the mass function is contained in the final $m'/m$, which can be computed (approximately) using Eqs. \ref{eq:convert_mass_c} and \ref{eq:convert_mass_m}.
However, it is important to notice the assumptions in this conversion: here we are preserving the total number density of halos (over all mass), but not the total mass density. That is, for each existing halo at some mass definition, we are reducing/extending that halo to encompass less/more mass, but not creating/destroying more halos from the remains. 
Importantly, this means that normalized HMFs will no longer obey this important halo model consistency relation for other mass definitions. 
Furthermore, there are significant errors introduced by such `translations' in comparison to properly re-fitting the mass function measured at the alternative definition (eg. fits from \mbox{%DIFAUXCMD
\citet{Bocquet2016} }\hspace{0pt}%DIFAUXCMD
suggest a 20\% error at low mass which diverges exponentially above the nonlinear mass). 
}\textsc{\DIFadd{hmf}} \DIFadd{allows automatically converting mass functions from their native mass definition to other definitions using this formalism, but this feature must be switched on manually by turning off the setting }\texttt{\DIFadd{disable\_mass\_conversion}} \DIFadd{(otherwise an inconsistent halo definition will cause an error to be raised).
Note that most fits that use spherical overdensity halos are directly calibrated to varying overdensity criteria \mbox{%DIFAUXCMD
\citep[eg.][]{Tinker2008,Watson2013,Bocquet2016}}\hspace{0pt}%DIFAUXCMD
, and }\textsc{\DIFadd{halomod}} \DIFadd{does not use the above conversion in these cases.
}

\DIFaddend We present a \DIFdelbegin \DIFdel{small selection of 7 out of }\DIFdelend \DIFaddbegin \DIFadd{selection of }\DIFaddend the 21 available fitting functions available in \textsc{hmf} in \DIFdelbegin \DIFdel{Figure \ref{fig:mfs}}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cref{fig:mfs}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% \begin{table*}
...
% \toprule[0.05cm]
%DIF <  \textbf{Available} &  $m$, $\nu$, $\delta_c$, $\sigma$, $n_\text{eff}$, $\ln\sigma^{-1}$, $z$, $\Delta_h$, \texttt{cosmo}, $\Omega_m(z)$ &  \\
%DIF >  \textbf{Available} &  $m$, $\nu$, $\delta_c$, $\sigma$, $n_\text{eff}$, $\ln\sigma^{-1}$, $z$, $\Delta_{\rm h}$, \texttt{cosmo}, $\Omega_m(z)$ &  \\
% \midrule
...
Table \ref{tab:models_bias} compiles the range of included models, and \DIFdelbegin \DIFdel{Figure \ref{fig:bias_functions} }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cref{fig:bias_functions} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend displays a selection of them.
...
\DIFaddbegin \DIFadd{Importantly, Table \ref{tab:models_bias} lists the paired HMF for each bias model, for which the consistency relation, Eq. \ref{eq:consistency}, holds. 
By default, }\textsc{\DIFadd{halomod}} \DIFadd{will choose the paired HMF once a bias function is chosen.
The paired HMF of any bias model can be determined by accessing the }\texttt{\DIFadd{pair\_hmf}} \DIFadd{attribute of the model.
}

\DIFadd{Note also that in the case that the HMF and bias function chosen support the consistency relation, by }\textit{\DIFadd{default}} \DIFadd{the large-scale 2-halo term of matter will be normalized to ensure it converges to $P_m(k)$ (i.e. a unity effective bias of matter). This needs to be manually enforced, because the numerical integration of the biased HMF does not necessarily converge to the mean matter density over the restricted range of mass included in the calculation. This normalization is implemented as a scalar multiplication of the entire 2-halo term, so as to avoid sharp features in the spectrum. This behaviour can be turned }\textit{\DIFadd{off}} \DIFadd{by setting }\texttt{\DIFadd{force\_unity\_dm\_bias}} \DIFadd{to false. 
}\DIFaddend %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% \begin{table}
% \centering
%  {\tabulinesep=1.3mm
...
%DIF >  \textbf{Available} &  $m$, $\nu$, $\delta_c$, $\Delta_{\rm h}$, $\Omega_m$, $n_s$, $M_\star$,$h$, $\sigma_8$ & Note that most of these parameters are unnecessary for most fits. \\
% \midrule
...
\begin{equation}
\DIFdelbegin \DIFdel{F^N_\alpha(x,y) = \frac{1}{2-\alpha}}%DIFDELCMD < \left[ %%%
\DIFdel{\frac{(x+y)^{2-\alpha}}{(1+x+y)^{2-\alpha}} - \frac{|x-y|^{2-\alpha}}{(1+|x-y|)^{2-\alpha}}}%DIFDELCMD < \right]%%%
\DIFdel{,
...
\begin{equation}
\DIFdelbegin \DIFdel{F^M_{3/2} = \frac{1}{3}}%DIFDELCMD < \left[ %%%
\DIFdel{2\sqrt{3} \tan^{-1}}%DIFDELCMD < \left(%%%
\DIFdel{\frac{-1+2\sqrt{z}}{\sqrt{3}}}%DIFDELCMD < \right) %%%
\DIFdel{+ \ln}%DIFDELCMD < \left(%%%
\DIFdel{\frac{1+2\sqrt{z} + z}{1-\sqrt{z}+z}}%DIFDELCMD < \right)\right]\Bigg|%%%
\DIFdel{_{|x-y|}^{x+y}.
}\DIFdelend \DIFaddbegin \begin{split}
...
% \toprule[0.05cm]
%DIF <  \textbf{Available} &  $\bar{\rho}$, $z$, $\Delta_h$, \texttt{\_cm\_relation} & The \texttt{\_cm\_relation} is a \texttt{CMRelation} model, with relevant methods.  \\
%DIF >  \textbf{Available} &  $\bar{\rho}$, $z$, $\Delta_{\rm h}$, \texttt{\_cm\_relation} & The \texttt{\_cm\_relation} is a \texttt{CMRelation} model, with relevant methods.  \\
% \midrule
%DIF <  \textbf{Required} & \texttt{\_f(x)} & $f(x)$, cf. Eq. \ref{eq:universalprofile} \\
%DIF >  \textbf{Required} & \texttt{\_f(x)} & $f(x)$, cf. \cref{eq:universalprofile} \\
% \midrule
%DIF <  \textbf{Optional} & \texttt{\_l(x,c)} & $l(x,c)$, cf. Eq. \ref{eq:lofxc} \\
%DIF >  \textbf{Optional} & \texttt{\_l(x,c)} & $l(x,c)$, cf. \cref{eq:lofxc} \\
% \midrule
%DIF <  \multirow{10}{*}{\textbf{Extra}} & \texttt{\_h(c)}* & $h(c)$, Eq. \ref{eq:hofc} \\
%DIF <  & \texttt{\_p(K,c)}* & $p(\kappa,c)$, Eq. \ref{eq:pofck} \\
%DIF >  \multirow{10}{*}{\textbf{Extra}} & \texttt{\_h(c)}* & $h(c)$, \cref{eq:hofc} \\
%DIF >  & \texttt{\_p(K,c)}* & $p(\kappa,c)$, \cref{eq:pofck} \\

% & \texttt{\_mvir\_to\_rvir(m)} & $r_\Delta(m)$ \\
% & \texttt{\_rvir\_to\_mvir(r)} & $m_\Delta(r)$ \\

% & \texttt{\_rs\_from\_m(m)}$\dagger$ & $r_s(m)$ \\
%DIF <  & \texttt{rho(r,m)}$\dagger\ddagger\triangledown$ & $\rho(r|m)$, Eq. \ref{eq:universalprofile}. \\
%DIF >  & \texttt{rho(r,m)}$\dagger\ddagger\triangledown$ & $\rho(r|m)$, \cref{eq:universalprofile}. \\
...
%DIF <  & \texttt{u(k,m)}$\dagger\ddagger$ & $u(k|m)$, Eq. \ref{eq:uspherical} \\
%DIF >  & \texttt{u(k,m)}$\dagger\ddagger$ & $u(k|m)$, \cref{eq:uspherical} \\

%DIF <  & \texttt{\_rho\_s(c)}$\ddagger$ &$\rho_s$, Eq. \ref{eq:hofm} \\
%DIF >  & \texttt{\_rho\_s(c)}$\ddagger$ &$\rho_s$, \cref{eq:hofm} \\
...
Note that the power-law fits provided are based on a particular cosmology, and will be inaccurate if used for a model with a different cosmology \DIFaddbegin \DIFadd{(the sensitivity to the cosmology is not high, and }\textsc{\DIFadd{halomod}} \DIFadd{does not warn the user if the cosmology does not match)}\DIFaddend . Thus, for general purpose applications we recommend an analytic model. 

\DIFdelbegin \DIFdel{Figure \ref{fig:concentration} }\DIFdelend \DIFaddbegin \DIFadd{Concentration-mass relations, as we have seen in \S\ref{sec:halomod:components:mass-def}, are sensitive to the halo definition. 
In principle, the concentrations can be `translated' from one definition to another \mbox{%DIFAUXCMD
\citep{Diemer2015} }\hspace{0pt}%DIFAUXCMD
given a halo profile. However, there is a significant scatter in this relationship, and it is better to work within the halo definition native to the measured concentration-mass relation.
For included concentration models in }\textsc{\DIFadd{halomod}}\DIFadd{, if the halo definition is not compatible with the measurement, a warning is issued -- but there is no attempt to translate the relation. This behaviour may be updated in the future to automatically translate the relation between halo definitions.
}


\DIFadd{\mbox{%DIFAUXCMD
\cref{fig:concentration} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend illustrates the variations between the models at $z=0$ for the default cosmology of \cite{PlanckCollaboration2015}. 

Note that \halomod\ includes an explicit interface for using the excellent suite of concentration-mass relations from \textsc{colossus}, which provides access to extra models such as the recent analytical models of \cite{Prada2012} and \cite{Diemer2019}. \DIFaddbegin \DIFadd{Importantly, models from }\textsc{\DIFadd{colossus}} \DIFadd{are able to translate between halo mass definitions. }\DIFaddend These models are plotted for comparison in \DIFdelbegin \DIFdel{\ref{fig:concentration}}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cref{fig:concentration}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend , but we do not include them in Table \ref{tab:models_concentration} and refer the reader to their respective papers.



%DIF <  \bd{Can we include models that are implemented in Colossus? At the moment, the model list for concentration looks a bit random...}
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  \bd{what are the weird upturns / downturns in the models in the Figure? Should they really be plotted that far out?}
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  \sgm{I put the analytic models from Colossus into the plot, and reduced the mass range plotted (the upturns etc. were from where the analytic models aren't solvable and a constant relation is returned). Do we want to list the colossus models in the table also?}
%DIFDELCMD < 
...
\DIFdelend \begin{figure}
  ...
  \caption[Concentration-mass-redshift relation for various models in the literature]{Illustration of the concentration-mass relations implemented in \halomod, focusing on analytic models (with the exception of the class power-law fit of \DIFdelbeginFL \DIFdelFL{\mbox{%DIFAUXCMD
\cite{Duffy2008}}\hspace{0pt}%DIFAUXCMD
}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\mbox{%DIFAUXCMD
\citealt{Duffy2008}}\hspace{0pt}%DIFAUXCMD
...
%DIF < The concentration-mass-redshift relation has a diversity of physical models, which makes implementing them in a uniform manner difficult. We begin the process with the most commonly used physical model of \citet{Bullock2001}, and the simple power-law approximations commonly used in observational studies (cf. \S\ref{sec:theory:concentration}).
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
%DIF < The common thread between physical models is the determination of some notion of ``formation time" of a halo \bd{I'd argue that's just one particular class of models...} \sgm{can discuss} -- whether it is the first time a certain fraction of the final mass is assembled, or the typical epoch at which a fraction of the mass collapsed, or some characteristic of the mass accretion history. For analytic models, this is usually obtained via the peak height or some close variant, so the filter function becomes an important quantity in these calculations. 
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF < Conversely, the power-law approximations generally require nothing more than the model parameters, though we provide the option to calculate the nonlinear mass if it is not passed explicitly.
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF < Our implementation of the \texttt{Bullock01} model is quite efficient, using interpolated splines to calculate the inverse of the growth factor, in order to acquire the collapse epoch:
%DIF < \begin{equation}
%DIF <     \label{eq:zc}
%DIF <     z_c = D^{-1}(\sqrt{\nu(Fm)}),
%DIF < \end{equation}
%DIF < where $D$ is the growth factor.
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF < Similarly, our implementation of the physical model of \citet{Ludlow2016} uses spline interpolation to perform root-finding. We do not present the full equations of that model here, for the sake of brevity, but refer the reader to the decsription in that paper.
%DIFDELCMD < 
...
%DIF <  \begin{table}
%DIF <  \centering
%DIF <   {\tabulinesep=1.3mm
%DIF <  \begin{tabu}{X[1l]X[1.5l]X[2l]} 
%DIF <  \toprule[0.05cm]
%DIF <  \textbf{Available} &  \texttt{filter}, \texttt{growth}, $\bar{\rho}_0$, $\delta_c$, $M_\star(z)$ & The bulk of these quantities will only be required for physical models. $M_\star$ may be passed explicitly as a model parameter, an instance variable, or left to be calculated in the class. \\
%DIF <  \midrule
%DIF <  \textbf{Required} & \texttt{cm(m,z)} & Return the concentration corresponding to $m$ at $z$ \\
%DIF <  \midrule
%DIF <  \textbf{Extra} & None & \\
%DIF <  \bottomrule[0.05cm]
%DIF <  \end{tabu}}
%DIF <  \caption{API summary of the \texttt{CMRelation} component.}
%DIF <  \label{tab:component_concentration}
%DIF <  \end{table}
%DIF < %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%DIFDELCMD < 
...
\toprule[0.05cm]
        ...
        \citet{Zehavi2011} & \texttt{Zehavi11} & & $a=11$, $b=-0.13$, $c=1$, $M_\star=2.26\times10^{12}$ \\
         ...
         \bottomrule[0.05cm]
\end{tabu}
\caption{Summary of concentration-mass-redshift relations implemented in \textsc{halomod}. Additional models can be imported from the \DIFdelbeginFL \textsc{\DIFdelFL{Colossus}} %DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \textsc{\DIFaddFL{colossus}} \DIFaddendFL code. \DIFaddbeginFL \DIFaddFL{Note that the relation from \mbox{%DIFAUXCMD
\citet{Duffy2008} }\hspace{0pt}%DIFAUXCMD
has multiple sets of parameters for different mass definitions and different halo samples. }\textsc{\DIFaddFL{halomod}} \DIFaddFL{includes all of these, and they can be manually specified and/or intelligently chosen.}\DIFaddendFL }
...
\toprule[0.05cm]
        ...
        \citet{Geach2012} & $\begin{aligned}&F_c^B(1-F_c^A)\exp \left[\frac{\log_{10}(m/M_c)^2}{2\sigma^2_{\log M}}\right] \\ &\times F_c^A \left[1+ \text{erf} \left(\frac{\log_{10}(m/M_c)}{\sigma_{\log M}}\right)\right]\end{aligned}$ & $F_s \left[1 + \text{erf} \left(\frac{\log_{10}(m/M_{min})}{\delta_{\log M}}\right)\right]\left(\frac{m}{M_{min}}\right)^\alpha$ \\ \midrule
         ...
         \citet{Contreras2013} & \DIFdelbeginFL \DIFdelFL{$N_c^\text{Geach}$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$N_{\rm c}^\text{Geach}$}\DIFaddendFL , $\sigma_{\log M} \rightarrow x\sigma_{\log M}$ & \DIFdelbeginFL \DIFdelFL{$N_s^\text{Geach}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$N_{\rm s}^\text{Geach}$ }\DIFaddendFL \\
         \bottomrule[0.05cm]
\end{tabu}
...
The primary task of the \verb|Exclusion| component is to evaluate \DIFdelbegin \DIFdel{Eq. \ref{eq:dmpower2a} (or Eq. \ref{eq:dmpower2} }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cref{eq:dmpower2a} }\hspace{0pt}%DIFAUXCMD
(or \mbox{%DIFAUXCMD
\cref{eq:dmpower2} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend if appropriate) and the associated (potentially modified) mean density, \DIFdelbegin \DIFdel{Eq. \ref{eq:theory:ng_dash}. }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cref{eq:theory:ng_dash}}\hspace{0pt}%DIFAUXCMD
. }\DIFaddend Each model, as discussed in \S\ref{sec:theory:exclusion}, is quite unique, and therefore there is not a great deal of redundancy that can be mitigate by class inheritance.
...
We show the effects of the halo exclusion for seven of the implemented models (two of them being modifications to \DIFdelbegin \DIFdel{$P_{hh}^c(k)$ }\DIFdelend \DIFaddbegin \DIFadd{$P_{\rm hh}^c(k)$ }\DIFaddend rather than physical halo exclusion models) in \DIFdelbegin \DIFdel{figure }\DIFdelend \DIFaddbegin \DIFadd{Fig. }\DIFaddend \ref{fig:halo_exclusion}. 
...
It is with these motivations that we present a successor to \textsc{HMFcalc}\footnote{\textsc{HMFcalc} was located at \url{https://hmf.icrar.org}. This address now automatically forwards to the address of \textsc{TheHaloMod}, at \url{https://thehalomod.app}. All the functionality of \textsc{HMFcalc} is available in \textsc{TheHaloMod}.} that computes the full range of halo model quantities we have outlined in this paper -- called \textsc{TheHaloMod}\DIFaddbegin \footnote{\DIFadd{The }\textit{\DIFadd{code}} \DIFadd{for }\textsc{\DIFadd{TheHaloMod}} \DIFadd{is open-source and available at }\url{https://github.com/halomod/TheHaloMod-SPA}\DIFadd{.}}\DIFaddend .


\subsection{Interface}
The interface of \thm\ is expansive but simple.
\DIFdelbegin \DIFdel{The user is presented immediately with a default model, which occupies a central plot. A }\DIFdelend \DIFaddbegin \DIFadd{It is a single-page application (SPA) that uses the }\textsc{\DIFadd{vue.js}}\footnote{\url{https://www.vuejs.org/}} \DIFadd{framework to render input forms dynamically.
The primary view presented to the user consists of a left-hand column containing an overview of the models the user has created and a }\DIFaddend drop-down menu \DIFdelbegin \DIFdel{lets the user select which quantity to view. 
Using a }%DIFDELCMD < \halomod%%%
\DIFdel{\ }%
{\color{red}%DIFAUXCMD
\verb|TracerHaloModel| %DIFAUXCMD
}%DIFAUXCMD
%DIFDELCMD < \verb|TracerHaloModel| %%%
\DIFdel{on the backend, these quantities are computed on-the-fly and then cached so they can be view again quickly. 
}\DIFdelend \DIFaddbegin \DIFadd{for downloading various data products, and a right-hand panel for viewing plots of arbitrary halo model quantities.
}\DIFaddend 

\DIFdelbegin \DIFdel{Below the plot itself is a table of computed models (consisting of just the default plot at first), each of which has a unique user-provided label.The user can remove or edit a model, or create a new model based on any existing model. Editing or model creation bring the user to }\DIFdelend \DIFaddbegin \DIFadd{The Models panel is initially empty until the user creates a model. 
Creating a model opens the input form view (technically this is on the same page, and overlays the primary view). We will discuss this view momentarily, but once a model has been defined, and a unique label is designated for it, the model appears in its own row in the Models panel on the main view. From here, the model may be renamed, edited (taking the user back to the input form view), cloned or deleted. 
Cloning the model makes a copy of model, but also takes the user to the input form view, to edit its parameters. 
In addition, there is an option for the user to report a bug specific to a particular model, which---along with a useful message from the user---collects and sends detailed information about the model to the developers. 
}

\DIFadd{The Download panel contains a single drop-down menu, offering choices of file downloads. Presently, the available options are (i) an image of }\DIFaddend the \DIFdelbegin \DIFdel{same interface containing a rather large input form.
The form itself is separated into tabs -- one for each }%DIFDELCMD < \framework%%%
\DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{plot (in SVG format), (ii) configuration files containing all of the parameters specified for each model (these come as ZIP archives, with one file per user-specified model, and each file can be in TOML or JSON format, where the TOML files are precisely in the format required to be input to the CLI, cf. \S\ref{sec:halomod:extra:cli}), and (iii) a ZIP archive containing ASCII data files with all possible halo model quantities stored in them (i.e. all quantities defined within the }\texttt{\DIFadd{TracerHaloModel}}\DIFadd{). The latter makes it simple for the user to produce their own plots with their preferred plotting tools. 
}

\DIFadd{The Plot panel contains a number of tools to customize what is being plotted.
Most importantly, there are two drop-downs that specify the quantities that comprise the $x$- }\DIFaddend and \DIFdelbegin \DIFdel{one for each }%DIFDELCMD < \component%%%
\DIFdel{. 
Each }%DIFDELCMD < \component%%%
\DIFdel{, in particular , has a }\DIFdelend \DIFaddbegin \DIFadd{$y$-axes. While typically the $x$-axis will either be halo mass, comoving scale or comoving wavenumber, more general quantities can be chosen (for example the peak-height $\nu$ instead of mass). The options available for the $y$-axis are dynamically updated based on the value of the $x$-axis, and include every possible quantity in the }\texttt{\DIFadd{TracerHaloModel}}\DIFadd{. Beyond this, there are options for setting the scale on either axis to be logarithmic (the default is intelligently chosen for each quantity).  Finally, the plot itself contains the chosen quantity for all models defined by the user. A legend to the right of the plot indicates corresponding models. 
}

\DIFadd{The interface is further available in both light and dark modes.
It is also fluid -- adjusting to fit on phone, tablet and monitor screens. 
}

\DIFadd{The input form view (cf. left-hand panel of Fig. \ref{fig:webpage-image}) presents all possible input parameters to a model. 
This is quite a few possible parameters -- including the choice of each kind of component model as well as all possible parameters to each of those models (eg. HOD models, bias models and HMF models). 
To make the interface concise and simple, the view consists of a left-hand panel that contains a dynamic index of parameter groups (essentially each of the components) so that particular parameters are easier to locate.
Each component has a similar layout for its inputs: a }\DIFaddend drop-down menu \DIFdelbegin \DIFdel{for choosing the particular }\DIFdelend \DIFaddbegin \DIFadd{to choose the }\DIFaddend model, and a \DIFdelbegin \DIFdel{unique set of input parameters for each model }\DIFdelend \DIFaddbegin \DIFadd{dynamically-rendered set of numerical inputs to change the unique parameters of the model (i.e. if }\texttt{\DIFadd{Tinker08}} \DIFadd{is chosen as the HMF model, only its parameters are shown to the user, and not those of any other mass function). 
The user is able to create and edit as many models as they wish, and these models persist between sessions. They are able to refresh and remove all their models via the Model panel on the main page}\DIFaddend .
\DIFdelbegin \DIFdel{Once parameters have been chosen (including the unique label), }\DIFdelend \DIFaddbegin 


\subsection{\DIFadd{Architecture}}
\DIFadd{The overall architecture of }\textsc{\DIFadd{TheHaloMod}} \DIFadd{is broken up into two parts: server and client. 
From a high-level perspective, the server handles the API portion of }\DIFaddend the \DIFdelbegin \DIFdel{user is taken back to the main plot interface, and is free to continue adding and editing models }\DIFdelend \DIFaddbegin \DIFadd{website, and the client holds the front-end. 
This way, the front-end can benefit from modern JavaScript UI libraries and the backend interfaces with the Python libraries }\textsc{\DIFadd{hmf}} \DIFadd{and }\textsc{\DIFadd{halomod}}\DIFaddend .

\DIFdelbegin \DIFdel{The user may also download various pieces of data: the plot itself, a readable list of parameters for each of the models, or data files containing }\textit{\DIFdel{all}} %DIFAUXCMD
\DIFdel{the quantities of the halo model for each model. 
}\DIFdelend \DIFaddbegin \DIFadd{Automatic testing on both server and client are performed via Github Actions, which also handle automatic continuous deployment on every commit to the main branch.
}\DIFaddend 

\DIFdelbegin \subsection{\DIFdel{Architecture}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < \thm%%%
\DIFdel{\ is powered by }\textsc{\DIFdel{django}}%DIFAUXCMD
\footnote{%DIFDELCMD < \url{https://www.djangoproject.com/}%%%
} %DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdel{and uses the }\textsc{\DIFdel{bootstrap}}%DIFAUXCMD
\footnote{%DIFDELCMD < \url{https://getbootstrap.com/}%%%
} %DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdel{CSS framework and }\textsc{\DIFdel{jQuery}}%DIFAUXCMD
\footnote{%DIFDELCMD < \url{https://jquery.com/}%%%
} %DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdel{Javascript library to achieve a reasonable UI with minimal fuss}\DIFdelend \DIFaddbegin \DIFadd{The server is built with the }\textsc{\DIFadd{flask}}\footnote{\url{https://flask.palletsprojects.com/en/2.0.x/}} \DIFadd{Python web framework. 
This framework handles the (very simple) API for the single-page app. 
Since it is a Python framework, it is able to interface directly with }\textsc{\DIFadd{halomod}}\DIFadd{. 
Models defined by the user (on the input form view) are serialized as a set of parameters, and used to construct }\texttt{\DIFadd{TracerHaloModel}} \DIFadd{objects on the server. These objects are serializable by construction, and are stored on the server (uniquely for each user) using }\textsc{\DIFadd{redis}}\footnote{\url{https://redis.io/}}\DIFadd{. 
Each time a quantity is requested by the user (to create the plots on the main view), each model object is deserialized, and the relevant quantity is computed (or obtained from cache if it has already been computed, cf. \S\ref{sec:halomod:overview:efficiency}). 
Cloning a model (via the Model panel on the main view) literally copies the instance of the model on the server, and uses the inherent caching and updating ability of }\textsc{\DIFadd{hmf}} \texttt{\DIFadd{Framework}} \DIFadd{objects to improve the speed of calculation of the new model.
In this way, the web-app essentially provides a thin, visual, wrapper around the full capability of a }\texttt{\DIFadd{TracerHaloModel}} \DIFadd{object.
}

\DIFadd{The frontend client is built with the }\textsc{\DIFadd{vue.js}} \DIFadd{web framework, and uses the }\textsc{\DIFadd{Material}}\footnote{\url{https://vuematerial.io/}} \DIFadd{CSS framework.
This provides a simple yet modern and flexible framework for both visual and interface design. 
The plots are created using }\texttt{\DIFadd{d3.js}}\footnote{\url{https://d3js.org/}}\DIFaddend .

\DIFdelbegin \DIFdel{Architecturally, }%DIFDELCMD < \thm%%%
\DIFdel{\ makes great use of the object-oriented forms in }\textsc{\DIFdel{django}}%DIFAUXCMD
\DIFdel{, in concert with the consistency of all the }\texttt{\DIFdel{Component}}%DIFAUXCMD
\DIFdel{s. 
This consistency means that very few lines of code are required to create separate form tabs for each }%DIFDELCMD < \component%%%
\DIFdel{. 
}\DIFdelend \DIFaddbegin \DIFadd{To enhance performance, an initial high-resolution object is pre-computed on the server, which is cloned and updated upon the first model definition by the user. In particular, this is very effective at producing high-resolution transfer functions from }\textsc{\DIFadd{camb}}\DIFadd{, which otherwise are computationally intensive.
}\DIFaddend 

\DIFdelbegin \DIFdel{Javascript functionality is kept to a minimum, but provides extremely useful UI improvements in some key cases. In particular, each }%DIFDELCMD < \component%%%
\DIFdel{\ model has potentially unique parameters. 
Javascript is used to make only relevant parameters visible to the user, dependent on their current choices.
It is also used to provide a smooth experience when changing the quantity to be viewed in the plot (i.
e. 
this can occur without reloading the page). }%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend \thm\ \DIFaddbegin \DIFadd{server }\DIFaddend is hosted by the Low-Frequency Cosmology (LOCO) Lab at Arizona State University. 
\DIFaddbegin \DIFadd{The client is separately hosted by }\textsc{\DIFadd{netlify}}\footnote{\url{https://d3js.org/}}\DIFadd{, which aids load speeds for users worldwide.
}\DIFaddend Deployment is aided by containerizing the entire application using Docker\footnote{\url{https://www.docker.com/}}.
...
%DIF <  \subsection{Example 2: Fitting Non-HOD parameters}
%DIF <  \label{sec:applications:nonhod}
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  In this example, we retain the HOD parameters from the previous example, but also attempt to fit two cosmological parameters: $\sigma_8$ and the spectral index $n_s$. 
%DIF <  Since we expect these to be poorly constrained by the observation, but highly constrained by priors from the CMB, we set fairly tight prior ranges on them.
%DIF <  The call to \verb|EnsembleSampler| remains almost exactly the same, except that \verb|'sigma_8'| and \verb|'n'| are added to the \verb|param_names|, \verb|ndim| is increased to 5, and we set \mintinline[breaklines,breakafter=:]{python}|bounds={"sigma_8":(0.75,0.85), "n":(0.94, 1.02)}|.
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  We show the resulting corner plot, this time omitting the derived scalar parameters for clarity, in Fig. \ref{fig:example2_corner}. {\color{red} COMMENTS}. The residual plot is shown in \ref{fig:example_resids}.
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  \subsection{Example 3: Non-Matched Model}
%DIF <  \label{sec:applications:non-matched}
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  In this example, we again retain the HOD parameters from the first example, but we assume that the profile model is the form of \citet{Einasto1965} (cf. Table \ref{tab:models_profile} and Fig. \ref{fig:profiles}). This profile is able to fit the NFW form quite closely, but not exactly, using $\alpha_{\rm Ein}\sim 0.18$. 
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  We let this parameter vary during the fit in this example, effectively using the galaxy correlation function inform us about the halo profile shape. We do not expect tight constraints on $\alpha_{\rm Ein}$, as systematic effects from the uncertain HOD parameters will be dominant. Thus, we set a reasonably tight prior on $\alpha_{\rm Ein}$ to avoid numerical issues, in particular we set \mintinline[breaklines,breakafter=:]{python}|bounds={'halo_profile_params.alpha':(0.05, 0.4)}| in the constructor to \verb|EnsembleSampler|. 
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  We show the resulting corner plot, this time omitting the derived scalar parameters for clarity, in Fig. \ref{fig:example3_corner}. {\color{red} COMMENTS}. The residual plot is shown in \ref{fig:example_resids}.
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  To illustrate the utility of \textsc{halomod} for common halo model applications, we here present an example motivated by typical analyses. In particular, we fit HOD models to the observed projected correlation function from the 6-degree Field Galaxy Survey \citep[6dFGS;][]{Jones2009}, in much the same fashion as the analysis in \citet[hereafter B13]{Beutler2013}\footnote{It must be noted that the observed correlation functions and their covariance matrices were kindly provided by Florian Beutler.}. 
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  \subsection{Setup and methodology}
%DIF <  Our aim is to begin by performing the same basic analysis as B13 (in a limited sense), but then extend it somewhat to show the flexibility of \halomod\  . Thus, we use the closest sample from their analysis, called ``S1", which has a mean redshift of $z=0.0369$ and mean density of $4.536\times10^{-3} h^3 \text{Mpc}^{-3}$, and use the inbuilt MCMC capabilities of \halomod\  to fit increasingly complex sets of variables with it. 
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  In all cases, we use the measured $w_p(r_p)$ from S1, with errors given by a covariance matrix measured with jack-knife resampling. We also use the mean galaxy density as a measurement to be fit, with a 1\% uncertainty.
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  All of our chains contain $10^5$ samples before the removal of burn-in, generated with 100 ensemble walkers. Burn-in length is determined by spectral methods, as 5 times the maximum autocorrelation length. The time-per-sample required for the runs on a standard quad-core machine running two cores per chain ranges from 1/40 to 1/10 seconds depending on the variables involved. 
%DIFDELCMD < 
...
%DIF <  \begin{table}
%DIF <      \centering
%DIF <      \begin{tabu} {X[3l]X[2l]}
%DIF <          \toprule[0.05cm] 
%DIF <          \textsc{Parameter} & \textsc{Value}  \\
%DIF <          \toprule[0.05cm]
        %DIFDELCMD < 
...
%DIF <          $T(k)$* & \texttt{EH} \\
%DIF <          $S(\xi_m(r))$ & \texttt{Tinker\_SD05} \\
%DIF <          $\rho(r|m)$ & NFW \\
%DIF <          \texttt{exclusion\_model} & \texttt{NgMatched\_} \\
%DIF <          \midrule
        %DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <          \texttt{lnk\_min}, \texttt{lnk\_max}, \texttt{dlnk} & -4, 2, 0.02 \\
%DIF <          \texttt{dlog10m} & 0.04 \\
%DIF <          \texttt{rnum} & 50 \\
%DIF <          \midrule 
                %DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <          \texttt{proj\_limit} & 50.0 \\
%DIF <          \texttt{z} & 0.0369 \\
%DIF <          \texttt{hc\_spectrum} & Nonlinear \\
%DIF <          \texttt{takahashi} & True \\    
%DIF <          \bottomrule[0.05cm]
%DIF <      \end{tabu}
%DIF <      \caption[Summary of base model for MCMC runs in example application]{Base model for our MCMC runs. The top section lists the various models used, the middle section lists the accuracy parameters, and the bottom section some extra options (some sample-specific). Note that the accuracy parameters were chosen to be as coarse as possible while only inducing a $<1\%$ error at any scale. Parameters marked with an asterisk (*) are known to differ from the analysis of B13.}
%DIF <      \label{tab:base_model}
%DIF <  \end{table}
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%DIF <  %  MCMC parameter table
%DIF <  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%DIF <  \begin{table*}
%DIF <      \centering
%DIF <      \begin{tabu} {X[2l]X[2l]X[1l]X[4l]}
%DIF <          \toprule[0.05cm] 
%DIF <          \textsc{Param} & \textsc{Prior} & \textsc{Baseline} & \textsc{Description} \\
%DIF <          \toprule[0.05cm]
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <          \multicolumn{3}{l}{\textsc{HOD}}    \\
%DIF <          $M_\text{min}$ & $[10,14]$ & & Minimum halo mass containing galaxies. \\
%DIF <          $M_1$ & $[10.85,15.2]$ & & Halo mass on average containing one satellite. \\
%DIF <          $\alpha$ &   $[0.5,1.7]$ & & Growth index of satellite occupation with halo mass \\
%DIF <          \midrule
        %DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <          \multicolumn{3}{l}{\textsc{Cosmology}*}    \\
%DIF <          $\sigma_8$ & $[0.4,1.1]$ & 0.846 & RMS density within spheres of 8$h^{-1}$Mpc \\
%DIF <          $n_s$ & $[0.7,1.3]$ & 0.9669  & Spectral index. \\
%DIF <          $\Omega_m$ &   $[0.2,0.4]$ & 0.3114 & Matter density today divided by critical density \\
%DIF <          $\Omega_b$ &   $[0.025,0.075]$ & 0.04888 & Baryon density today divided by critical density \\
%DIF <          $H_0$ & $[55.0,85.0]$ & 67.58 & Hubble constant today. \\
%DIF <          \midrule
        %DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <          \multicolumn{3}{l}{\textsc{HMF}}    \\
%DIF <          $\alpha_0$ & \multirow{5}{*}{$\mathcal{N}(\mu,10\%)$} & 0.368 & \multirow{5}{*}{\parbox{5.5cm}{$z=0$ parameters of the \citet{Tinker2010} fit.}} \\
%DIF <          $\beta_0$ & & 0.589 & \\
%DIF <          $\gamma_0$ & & 0.864 & \\
%DIF <          $\phi_0$ & & -0.729 & \\
%DIF <          $\eta_0$ & & -0.243 & \\        
...
%DIFDELCMD < %%%
%DIF <          \multicolumn{3}{l}{\textsc{Bias}}    \\
%DIF <          $B$ & \multirow{3}{*}{$\mathcal{N}(\mu,10\%)$} & 0.183 & \multirow{3}{*}{\parbox{5.5cm}{$z=0$ parameters of the \citet{Tinker2010} bias.}} \\
%DIF <          $b$ & & 1.5 & \\
%DIF <          $c$ & & 2.4 & \\
%DIF <          \midrule        
%DIFDELCMD < 
...
%DIFDELCMD < %%%
%DIF <          \multicolumn{3}{l}{\textsc{Concentration}}
        %DIFDELCMD < 

%DIFDELCMD <         
%DIFDELCMD < %%%
%DIF <          \\
%DIF <          $a$ & \multirow{3}{*}{$\mathcal{N}(\mu,10\%)$} & 6.71 & \multirow{3}{*}{\parbox{5.5cm}{Parameters of the \citet{Duffy2008} $c(m,z)$ relation}} \\
%DIF <          $b$ & & -0.091 & \\
%DIF <          $c$ & & 0.44 & \\
        %DIFDELCMD < 
...
%DIF <   \begin{figure*}
%DIF <  \centering
%DIF <  \includegraphics[width=\linewidth]{with_cosmo_cov_triangle.pdf} 
...
Up-to-date planned improvements for all of these codes can be found by browsing their issues on GitHub\footnote{Eg. \DIFdelbegin %DIFDELCMD < \url{https://github.com/steven-murray/halomod/issues}%%%
...
The transformation from 3D power spectrum to real-space correlation function is given by \DIFdelbegin \DIFdel{Eq. \ref{eq:hankel}}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cref{eq:hankel}}\hspace{0pt}%DIFAUXCMD
...
In the case of the zeroth-order spherical Hankel transform, we may make some simplifications. We firstly take \DIFdelbegin \DIFdel{Eq. \ref{eq:hankel} }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cref{eq:hankel} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend and make the substitution $z=kr$:
\begin{equation}
	\label{eq:hankelsub}
	\xi(r) = \frac{1}{2\pi^2r^3}\int_0^\infty P(z/r)z^2j_0(z)\DIFdelbegin \DIFdel{dz}\DIFdelend \DIFaddbegin {\rm \DIFadd{d}}\DIFadd{z}\DIFaddend ,
\end{equation} 
...
\end{equation}
so we may update \DIFdelbegin \DIFdel{Eq. \ref{eq:hankelsub} to
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cref{eq:hankelsub} }\hspace{0pt}%DIFAUXCMD
to
}\DIFaddend \begin{equation}
	\label{eq:finalhankel}
	\xi(r) = \frac{1}{2\pi^2r^3}\int_0^\infty P(z/r) z^2\sqrt{\frac{\pi}{2z}}J_{1/2}(z)\DIFdelbegin \DIFdel{dz}\DIFdelend \DIFaddbegin {\rm \DIFadd{d}}\DIFadd{z}\DIFaddend .
\end{equation}
\DIFdelbegin \DIFdel{Eq. \ref{eq:finalhankel} }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cref{eq:finalhankel} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend is in the form required to implement the method of \cite{Ogata2005}, with $\nu=1/2$. In this case, the zeros of the function, $r_{\nu k} = 1,2,3...$. 
...
Since the values, $y_{\nu k}$, at which the sum is evaluated approach the zeros of the bessel function as $k \rightarrow 0$, \citet{Murray2019} found that a maximum of $N=3.2/h$ steps are required (for reasonably small $h$).
 ...
 To avoid potential memory issues, we sum \DIFdelbegin \DIFdel{Eq. \ref{eq:hankel_final_identity} }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cref{eq:hankel_final_identity} }\hspace{0pt}%DIFAUXCMD
 ...
 For scales $k < 10^{-2}$, the 2-halo term may be very well approximated by the linear power multiplied by the effective bias (\DIFdelbegin \DIFdel{Eq. \ref{eq:beff}}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cref{eq:beff}}\hspace{0pt}%DIFAUXCMD
...
The finite lower limit has more difficult convergence properties. We make the substitution $y=r-r_p$ in \DIFdelbegin \DIFdel{Eq. \ref{eq:wprp} }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cref{eq:wprp} }\hspace{0pt}%DIFAUXCMD
